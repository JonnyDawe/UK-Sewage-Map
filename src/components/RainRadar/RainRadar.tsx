    ...
    (tPath: RainviewerItem) => {
      if (view !== null) {
        ...
        if (pastTimePathRef.current && pastTimePathRef.current.time !== tPath.time) {
          oldLayers = findLayersByIdKeyword(view.map.layers, `${pastTimePathRef.current.time}`);
        }
  ...
  React.useEffect(() => {
    if (radarTimePaths) {
      setCurrentTimePath((current) =>
        current === null ? (radarTimePaths.past[radarTimePaths.past.length - 1] ?? null) : current,
      );
