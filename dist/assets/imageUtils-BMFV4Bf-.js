import { l4 as m, b5 as R, er as v, A as V, R as _ } from './index.lazy-BFilFZ3v.js';
import { S as y, p as I } from './BitmapTechnique-FMpr9zIt.js';
import { r as M } from './WGLContainer-CljHC-xu.js';
import { i as A } from './TileContainer-DUIdLrEG.js';
let C = class extends M {
    constructor(e, i, r, s, n, o, a = null) {
      super(e, i, r, s, n, o),
        (this.bitmap = new y(a)),
        (this.bitmap.coordScale = [n, o]),
  ...
  q = class extends A {
    ...
    createTile(e) {
      const i = this.tileInfoView.getTileBounds(R(), e),
        r = this.tileInfoView.getTileResolution(e.level),
        [s, n] = this.tileInfoView.tileInfo.size;
      return new C(e, r, i[0], i[3], s, n);
    }
    onAttach() {
      super.onAttach(), (this._bitmapTechnique = new I());
    }
    onDetach() {
      var e;
      super.onDetach(),
        (e = this._bitmapTechnique) == null || e.shutdown(),
        (this._bitmapTechnique = null);
    }
    renderChildren(e) {
      if (
        (super.renderChildren(e),
        !this.visible || e.drawPhase !== v.MAP || this._bitmapTechnique == null)
      )
        return;
      const i = this.children.map((r) => r.bitmap);
  ...
  let e = class extends t {
    attach() {
      this.view.timeline.record(`${this.layer.title} (BitmapTileLayer) Attach`),
        (this._bitmapView = new q(this._tileInfoView)),
    ...
    detach() {
      ...
      this.container.removeChild(this._bitmapView),
        ...
        (this._bitmapView = null);
    }
  };
  return (e = V([_('esri.views.2d.layers.BitmapTileLayerView2D')], e)), e;
};
function D(t) {
...
function z(t, e, i, r) {
  ...
  const s = t.tileInfo.size,
    n = t.getTileResolution(i.level),
    o = t.getTileResolution(r.level);
  let a = t.getLODInfoAt(r.level);
  const d = a.getXForColumn(r.col),
    u = a.getYForRow(r.row);
  a = t.getLODInfoAt(i.level);
  const p = a.getXForColumn(i.col),
    f = a.getYForRow(i.row),
    l = D(e) / s[0],
    h = F(e) / s[1],
    b = Math.round(l * ((d - p) / n)),
    w = Math.round(h * (-(u - f) / n)),
    g = Math.round(l * s[0] * (o / n)),
    T = Math.round(h * s[1] * (o / n)),
    c = L(s);
  return c.getContext('2d').drawImage(e, b, w, g, T, 0, 0, s[0], s[1]), c;
}
function L(t) {
  const e = document.createElement('canvas');
  return ([e.width, e.height] = t), e;
}
export { z as n, L as o, O as r };
