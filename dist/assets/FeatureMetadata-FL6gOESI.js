import { s as le, t as Y, i as ce } from './dataViewUtils-Dzpw_nGB.js';
import {
  r2 as _e,
  dO as fe,
  r3 as pe,
...
import { s as Re, n as Be, p as Me, f as w, q as Oe, r as _ } from './UpdateTracking2D-FxGawpd5.js';
import { B as $e } from './queryUtils-DePK7lxY.js';
import { I as Ge, l as ke } from './timeSupport-JKWk7KR2.js';
import {
  u as V,
  F as Ve,
  M as K,
...
import { i as We, a as Ue } from './quantizationUtils-QwHxl707.js';
function Pe(a, e) {
  if (a == null) return '';
  const t = e.domain;
  if (t) {
    if (t.type === 'codedValue' || t.type === 'coded-value') {
      ...
      const { max: i, min: s } = _e(e),
        n = +a;
      if (s != null && i != null && s <= n && n <= i) return t.name;
    }
  }
  let r = a;
  return fe(e) ? (r = pe(r, ye('short-date'))) : me(e) && (r = ge(+r)), r || '';
}
let se = class R {
  ...
  hasRange(e, t) {
    ...
    if (r === i) return !1;
    for (let s = r / 32; s !== i / 32; s++) if (this._buf[s]) return !0;
    return !1;
  }
  set(e) {
    const t = this._mask & e,
  ...
  resize(e) {
    const t = this._buf,
      r = new Uint32Array(Math.ceil(e / 32));
    r.set(t), (this._buf = r);
  }
  or(e) {
  ...
  _ensureSize(e) {
    if (this._pos + e >= this._buffer.length) {
      const t = this._roundToNearest(
          (this._array.byteLength + e * this._buffer.BYTES_PER_ELEMENT) * Ze,
...
let Xe = class {
    constructor(e, t, r = 0) {
      const i = Ye * r * Uint32Array.BYTES_PER_ELEMENT,
        s = He * r * t.stride,
        n = t.stride / 4,
        o = t.attributes.find((h) => h.name === 'pos' || h.name === 'position');
      if (!o) throw new Error('InternalError: Unable to find position attribute');
      (this.layout = { ...t, position: o }),
        (this._indices = new B(Uint32Array, i)),
        (this._vertices = new B(Uint32Array, s)),
        (this._metrics = new B(Uint32Array, 0)),
        (this._metricCountOffset = this._metrics.push(0)),
        (this._strideInt = n),
        (this._instanceId = e);
    }
    serialize(e) {
      const t = this._indices.buffer(),
        r = this._vertices.buffer(),
        i = this._metrics.length ? this._metrics.buffer() : null;
      return (
        e.push(t, r),
        { instanceId: this._instanceId, layout: this.layout, indices: t, vertices: r, metrics: i }
      );
    }
    get strideInt() {
  ...
  Mt = class {
    constructor(e, t = 0) {
      (this._id = e),
        (this._sizeHint = t),
        (this._entityRecordCountOffset = 0),
        (this._entityCountOffset = 0),
        (this._entityIdIndex = 0),
        (this._entitySortKeyIndex = 0),
        (this._didEntityStart = !1),
        (this._instanceIdToVertexData = new Map()),
        (this._recordIndexStart = 0),
        (this._recordIndexCount = 0),
        (this._recordVertexStart = 0),
        (this._recordVertexCount = 0),
        (this._current = {
          metric: null,
          writer: null,
          start: 0,
        ...
        (this._entities = new B(Uint32Array, this._sizeHint * le.byteSizeHint)),
    ...
    vertexCount() {
      var e;
      return ((e = this._current.writer) == null ? void 0 : e.vertexCount) ?? 0;
    }
    indexCount() {
      var e;
      return ((e = this._current.writer) == null ? void 0 : e.indexCount) ?? 0;
    }
    vertexEnsureSize(e) {
    ...
    entityStart(e, t = e) {
      (this._entityIdIndex = this._entities.push(e)),
        (this._entitySortKeyIndex = this._entities.writeF32(t)),
        (this._entityRecordCountOffset = this._entities.push(0)),
        (this._didEntityStart = !0);
    }
    entityRecordCount() {
      return this._entities.getValue(this._entityRecordCountOffset);
    }
    entityEnd() {
      this._didEntityStart &&
        (this.entityRecordCount() === 0
          ? this._entities.seek(this._entityIdIndex)
          : this._entities.incr(this._entityCountOffset),
        (this._didEntityStart = !1));
    }
    recordCount() {
      return this._entities.getValue(this._entityRecordCountOffset);
    ...
    recordStart(e, t, r = 0) {
      (this._current.writer = this._getVertexWriter(e, t)),
        (this._current.indexStart = this._current.writer.indexCount),
        (this._current.vertexStart = this._current.writer.vertexCount),
        (this._current.instanceId = e),
        (this._current.layoutHash = t.hash),
        (this._current.textureKey = r);
    }
    recordEnd(e = 0) {
      const t = this._current.vertexStart,
        r = this._current.writer.vertexCount - t;
      if (!r) return !1;
      const i = this._current.indexStart,
        s = this._current.writer.indexCount - i;
      return (
        (this._recordIndexStart = i),
        (this._recordIndexCount = s),
        (this._recordVertexStart = t),
        (this._recordVertexCount = r),
        this._entities.incr(this._entityRecordCountOffset),
        Y.write(this._entities, this._current.instanceId, this._current.textureKey, i, s, t, r, e),
        !0
      );
    }
    copyLast(e, t) {
      ...
      this._entities.incr(this._entityRecordCountOffset),
        Y.write(
          this._entities,
          this._current.instanceId,
          this._current.textureKey,
          this._recordIndexStart + this._recordIndexCount,
          this._recordIndexCount,
          r,
          this._recordVertexCount,
          0,
        );
      const i = this._current.writer.indexWriter,
        s = this._current.writer.vertexWriter,
        n = this._recordIndexStart + this._recordIndexCount,
        o = this._recordVertexCount;
      for (let c = this._recordIndexStart; c !== n; c++) {
        const d = i.getValue(c);
        i.push(d + o);
      }
      const h = this._current.writer.layout.stride / Uint32Array.BYTES_PER_ELEMENT,
        u = this._recordVertexStart * h,
        l = (this._recordVertexStart + this._recordVertexCount) * h;
      for (let c = u; c !== l; c++) {
        const d = s.getValue(c);
        s.push(d);
      }
      const f = this._current.writer.layout.position,
        y = f.packPrecisionFactor ?? 1,
        I = f.offset / Uint32Array.BYTES_PER_ELEMENT,
        S = e * y,
        p = t * y;
      for (let c = r * h; c <= s.length; c += h) s.i1616Add(c + I, S, p);
    }
    copyLastFrom(e, t, r) {
      const i = e._entities.getValue(e._entityIdIndex);
      if (i !== this._entities.getValue(this._entityIdIndex)) {
        const d = e._entities.getValueF32(e._entitySortKeyIndex);
        this.entityStart(i, d);
      }
      this.recordStart(e._current.instanceId, e._current.writer.layout, e._current.textureKey);
      const s = this._current.writer.layout.stride / Uint32Array.BYTES_PER_ELEMENT,
        n = this._current.vertexStart,
        o = e._current.vertexStart - n,
        h = this._current.writer.indexWriter,
        u = this._current.writer.vertexWriter,
        l = e._current.writer.indexWriter,
        f = e._current.writer.vertexWriter;
      for (let d = e._current.indexStart; d !== l.length; d++) {
        const G = l.getValue(d);
        h.push(G - o);
      }
      for (let d = e._current.vertexStart * s; d !== f.length; d++) {
        const G = f.getValue(d);
        u.push(G);
      }
      const y = this._current.writer.layout.position,
        I = y.packPrecisionFactor ?? 1,
        S = y.offset / Uint32Array.BYTES_PER_ELEMENT,
        p = t * I,
        c = r * I;
      for (let d = n * s; d <= u.length; d += s) u.i1616Add(d + S, p, c);
      this.recordEnd();
    }
    _getVertexWriter(e, t) {
      const r = this._instanceIdToVertexData;
      return r.has(e) || r.set(e, new Xe(e, t, this._sizeHint)), r.get(e);
    }
  };
function Je(a) {
  return 'url' in a && 'urlHash' in a ? { ...a, url: '' } : a;
}
let Ke = class {},
  Gt = class extends Ke {
    constructor(e) {
      super(),
        ...
        (this._resourceIdToResource = new Map());
    }
    destroy() {
    ...
    enqueueRequest(e) {
      const t = Je(e.resource),
        r = re(JSON.stringify(t));
      return (
        this._pendingIds.has(r) ||
          (this._pendingIds.add(r), this._pendingRequests.push({ ...e, resourceId: r })),
        r
      );
    }
    async fetchEnqueuedResources() {
      const e = this._pendingRequests;
      if ((this._pendingIds.clear(), (this._pendingRequests = []), e.length === 0)) return;
...
const Qe = (a, e) => (a && ((...t) => e.warn('DEBUG:', ...t))) || (() => null),
  qe = !1;
let et = class {
    constructor(e) {
  ...
  tt = class {
    constructor() {
      (this._freeIdsGenerationA = []),
        (this._freeIdsGenerationB = []),
        (this._idCounter = 1),
        (this._freeIds = this._freeIdsGenerationA),
    ...
    createIdForObjectId(e) {
      ...
      return (
        t
          ? t.increment()
          : ((t = new et(Re(this._getFreeId(), !1))),
            t.increment(),
            this._objectIdToDisplayId.set(e, t)),
        t.data
      );
    }
    releaseIdForObjectId(e) {
      ...
      t &&
        (t.decrement(),
        t.empty() && (this._objectIdToDisplayId.delete(e), this._freeIds.push(t.data)));
    }
  ...
  rt = class {
    constructor(e) {
      (this._valid = se.create(e)), (this._data = new Array(e));
    }
    has(e) {
...
const O = m('featurelayer-simplify-thresholds') ?? [0.5, 0.5, 0.5, 0.5],
  it = O[0],
  st = O[1],
  nt = O[2],
  at = O[3],
  P = m('featurelayer-simplify-payload-size-factors') ?? [1, 2, 4],
  ot = P[0],
  ht = P[1],
  ut = P[2],
  dt = m('featurelayer-simplify-mobile-factor') ?? 2,
  lt = m('esri-mobile'),
  Q = 4294967295;
function ct(a, e, t) {
  if (!(a.length > e)) for (; a.length <= e; ) a.push(t);
}
class _t {
  constructor(e) {
    (this.metadata = e),
  ...
  getAreaSimplificationThreshold(e, t) {
    let r = 1;
    const i = lt ? dt : 1;
    t > 4e6 ? (r = ut * i) : t > 1e6 ? (r = ht * i) : t > 5e5 ? (r = ot * i) : t > 1e5 && (r = i);
    let s = 0;
    return (
      e > 4e3 ? (s = at * r) : e > 2e3 ? (s = nt * r) : e > 100 ? (s = st) : e > 15 && (s = it), s
    );
  }
  getBounds(e) {
    if ((ct(this._boundsBuffer, 4 * this.getIndex() + 4, 0), this.getBoundsXMin() === Q)) return !1;
    if (this.getBoundsXMin() === 0) {
      const n = this.readGeometryWorldSpace();
      if (!n) return this.setBoundsXMin(Q), !1;
      let o = 1 / 0,
        h = 1 / 0,
        u = -1 / 0,
        l = -1 / 0;
      return (
        n.forEachVertex((f, y) => {
          (o = Math.min(o, f)), (h = Math.min(h, y)), (u = Math.max(u, f)), (l = Math.max(l, y));
        }),
        this.setBoundsXMin(o),
        this.setBoundsYMin(h),
        this.setBoundsXMax(u),
        this.setBoundsYMax(l),
        W(e, o, h, u, l),
        !0
      );
    }
    const t = this.getBoundsXMin(),
      ...
      i = this.getBoundsXMax(),
      s = this.getBoundsYMax();
    return W(e, t, r, i, s), !0;
  }
  getBoundsXMin() {
  ...
  readOptimizedFeatureWorldSpace() {
    ...
    return new ie(e, t, r, this.getObjectId(), this.getDisplayId());
  }
  ...
  readLegacyGeometryForDisplay() {
    ...
    return V(e, this.geometryType, !1, !1);
  }
  readXForDisplay() {
    return this._readX();
  }
  ...
  readGeometryForDisplay() {
    ...
    if (!e) {
      const t = this._createDeltaQuantizedGeometryFromServerCentroid();
  ...
  readGeometryForDisplayTransformed(e) {
    ...
    if ((t && (t = Ve(t, e, this.metadata.geometryType, this.hasZ, this.hasM)), !t)) {
      const r = this.readCentroidForDisplay();
      if (!r) return null;
      const i = We(e, r.coords[0]),
        s = Ue(e, r.coords[1]);
      return this._createDeltaQuantizedExtrudedGeometry(i, s).deltaDecode();
    }
    return t;
  }
  readGeometryWorldSpace() {
    ...
    return r != null && K(t, t, this.hasZ, this.hasM, r), t;
  }
  ...
  readCentroidWorldSpace() {
    ...
    if (!t) return null;
    const r = t.clone(),
      i = this.getInTransform();
    return i != null && K(r, r, this.hasZ, this.hasM, i), r;
  }
  setCache(e) {
    let t = this._caches.get(e);
    t == null && ((t = new rt(this.getSize())), this._caches.set(e, t)), (this._activeCache = t);
  }
  setCachedValue(e) {
  ...
  geometry() {
    const e = this.readGeometryWorldSpace(),
      t = V(e, this.geometryType, this.hasZ, this.hasM),
      r = be(t);
    if (r) {
      if (!this.metadata.outSpatialReference)
  ...
  autocastArcadeDate(e, t) {
    return t && t instanceof Date
      ? this.isUnknownDateTimeField(e)
        ? C.unknownDateJSToArcadeDate(t)
        : C.dateJSAndZoneToArcadeDate(t, this.contextTimeZone ?? Ie)
      : t;
  }
  isUnknownDateTimeField(e) {
    return this.metadata.fieldsIndex.getTimeZone(e) === xe;
  }
  field(e) {
    ...
    if (t)
      switch (t.type) {
        ...
        case 'timestamp-offset':
          return C.fromReaderAsTimeStampOffset(this.readAttribute(e, !1));
        case 'date':
    ...
    for (const r of this._joined)
      if ((r.setIndex(this.getIndex()), (t = r.fields.get(e)), t))
        switch (t.type) {
          case 'date-only':
          case 'esriFieldTypeDateOnly':
          ...
          case 'esriFieldTypeTimestampOffset':
          case 'timestamp-offset':
            return C.fromReaderAsTimeStampOffset(r._readAttribute(e, !1));
          case 'date':
          case 'esriFieldTypeDate':
    ...
    return {
      ...
      geometry:
        (e == null ? void 0 : e.keepGeometryType) === !0
          ? this.geometry()
          : (((t = this.geometry()) == null ? void 0 : t.toJSON()) ?? null),
    };
  }
  ...
  _computeDisplayCentroid(e) {
    if (this.getInTransform() == null) return we(new E(), e, this.hasM, this.hasZ);
    const t = Be.fromOptimized(e, this.geometryType);
    t.yFactor *= -1;
    const r = Me(t);
    return r ? ((r[1] *= -1), new E([], r)) : null;
  }
  copyInto(e) {
    (e._joined = this._joined),
      (e._overrides = this._overrides),
      (e._objectIdToIndex = this._objectIdToIndex),
      (e._boundsBuffer = this._boundsBuffer),
      (e._activeCache = this._activeCache),
      (e._caches = this._caches),
  ...
  _createDeltaQuantizedExtrudedGeometry(e, t) {
    return this.geometryType === 'esriGeometryPolyline'
      ? this._createDeltaQuantizedExtrudedLine(e, t)
      : this._createDeltaQuantizedExtrudedQuad(e, t);
  }
  _createDeltaQuantizedExtrudedQuad(e, t) {
    return new E([5], [e - 1, t, 1, -1, 1, 1, -1, 1, -1, -1]);
  }
  _createDeltaQuantizedExtrudedLine(e, t) {
    return new E([2], [e - 1, t + 1, 1, -1]);
  }
}
class g extends _t {
  static fromFeatures(e, t) {
    const { objectIdField: r, geometryType: i } = t,
      s = je([], e, i, !1, !1, r);
    for (let n = 0; n < s.length; n++) s[n].displayId = e[n].displayId;
    return g.fromOptimizedFeatures(s, t);
  }
  static fromFeatureSet(e, t) {
    const r = Ne(e, t.objectIdField);
    return g.fromOptimizedFeatureSet(r, t);
  }
  static fromOptimizedFeatureSet(e, t) {
    const r = g.fromOptimizedFeatures(e.features, t);
    return (
      (r._exceededTransferLimit = e.exceededTransferLimit),
      (r._transform = e.transform),
      (r._fieldsIndex = new M(e.fields)),
      r
    );
  }
  static fromOptimizedFeatures(e, t, r) {
    const i = new g(e, t);
    return (i._fieldsIndex = t.fieldsIndex), (i._transform = r), i;
  }
  static empty(e) {
    return new g([], e);
  }
  constructor(e, t) {
    super(t),
      ...
      (this._geometryType = t.geometryType),
      (this._features = e);
  }
  get fields() {
  ...
  getObjectId() {
    var e;
    return (e = this._current) == null ? void 0 : e.objectId;
  }
  getDisplayId() {
  ...
  readGeometryArea() {
    return D(this._current) ? Le(this._current.geometry, 2) : 0;
  }
  _readX() {
    return D(this._current) ? this._current.geometry.coords[0] : 0;
  }
  _readY() {
    return D(this._current) ? this._current.geometry.coords[1] : 0;
  }
  _readGeometry() {
    return D(this._current) ? (this._current.geometry ?? null) : null;
  }
  _readServerCentroid() {
  ...
  _readAttribute(e, t) {
    if (!this._fieldsIndex) {
      ...
      for (const o in this._current.attributes)
        if (o.toLowerCase() === n) return this._current.attributes[o];
      return;
    }
    const r = this._fieldsIndex.get(e);
    if (!r) return;
    const i = this._current.attributes[r.name];
    return i == null ? i : t && this.fields.isDateField(e) ? new Date(i) : i;
  }
  _readAttributes() {
  ...
  copyInto(e) {
    super.copyInto(e),
      ...
      (e._fieldsIndex = this._fieldsIndex);
  }
}
let U = class {
  getObjectId(e) {
  ...
  cloneWithGeometry(e, t) {
    const r = e.readAttributes(),
      i = new ie(t, r, null, e.getObjectId(), e.getDisplayId()),
      s = g.fromOptimizedFeatures([i], e.metadata);
    return s.setIndex(0), s;
  }
  getGeometry(e) {
...
U.Shared = new U();
const j = () => A.getLogger('esri.views.2d.layers.FeatureLayerView2D'),
  ft = { getAttribute: (a, e) => a.readAttribute(e) };
async function ne(a, e) {
  try {
    const t = await Se(a, e);
    return (
      t.isStandardized ||
        j().error(new b('sql-parse-error', 'expression is not standardized', { where: a })),
      (r, i) => {
        const s = r.readArcadeFeature();
        try {
          return t.testFeatureCompiled(s, ft, i.currentUser);
        } catch (n) {
          return (
            j().warn(
              new b('sql-runtime-error', 'Encountered an error when evaluating where clause', {
                where: a,
                error: n,
              }),
            ),
...
let yt = class ae {
  constructor(e) {
    (this._geometryBounds = Te()), (this._idToVisibility = new Map()), (this._serviceInfo = e);
  }
  ...
  setKnownIds(e) {
    for (const t of e) this._idToVisibility.set(t, N);
  }
  setTrue(e) {
    const t = [],
      r = [],
      i = new Set(e);
    return (
      this._idToVisibility.forEach((s, n) => {
        const o = !!(this._idToVisibility.get(n) & N),
          h = i.has(n);
        !o && h ? t.push(n) : o && !h && r.push(n), this._idToVisibility.set(n, h ? N | pt : 0);
      }),
      { show: t, hide: r }
    );
  }
  createQuery() {
    ...
    return Fe.fromJSON({ geometry: e, spatialRel: t, where: r, timeExtent: i, objectIds: s });
  }
  async update(e, t) {
    this._hash = JSON.stringify(e);
    const r = await $e(e, null, t);
    await Promise.all([
      this._setGeometryFilter(r),
      this._setIdFilter(r),
      this._setAttributeFilter(r),
      this._setTimeFilter(r),
    ]);
  }
  async _setAttributeFilter(e) {
    if (!(e != null && e.where)) return (this._clause = null), void (this.where = null);
    (this._clause = await ne(e.where, this._serviceInfo.fieldsIndex)), (this.where = e.where);
  }
  _setIdFilter(e) {
    (this._idsToShow = (e == null ? void 0 : e.objectIds) && new Set(e.objectIds)),
      (this._idsToHide = (e == null ? void 0 : e.hiddenIds) && new Set(e.hiddenIds)),
      (this.objectIds = e == null ? void 0 : e.objectIds);
  }
  async _setGeometryFilter(e) {
    if (!(e != null && e.geometry))
      return (
        (this._spatialQueryOperator = null), (this.geometry = null), void (this.spatialRel = null)
      );
    const t = e.geometry,
      r = e.spatialRel ?? 'esriSpatialRelIntersects',
      i = await Ge(
        r,
        t,
    ...
    Ae(this._geometryBounds, t),
      (this._spatialQueryOperator = i),
      (this.geometry = t),
      (this.spatialRel = r);
  }
  _setTimeFilter(e) {
    if (((this.timeExtent = this._timeOperator = null), e == null ? void 0 : e.timeExtent))
      if (this._serviceInfo.timeInfo)
        (this.timeExtent = e.timeExtent),
          (this._timeOperator = ke(this._serviceInfo.timeInfo, e.timeExtent, U.Shared));
      else {
        const t = new b(
        ...
        A.getLogger('esri.views.2d.layers.features.controllers.FeatureFilter').error(t);
      }
  }
  _applyFilter(e, t) {
    return (
      this._filterByGeometry(e) &&
      this._filterById(e) &&
      this._filterByTime(e) &&
      this._filterByExpression(e, t)
    );
  }
  _filterByExpression(e, t) {
    return !this.where || this._clause(e, t);
  }
  _filterById(e) {
    var t, r;
    return (
...
const Z = () => A.getLogger('esri.views.layers.2d.features.support.AttributeStore'),
  L = Qe(qe, Z()),
  gt = { sharedArrayBuffer: m('esri-shared-array-buffer'), atomics: m('esri-atomics') };
class bt {
  constructor(e, t, r) {
    (this.size = 0), (this.texelSize = 4), (this.dirtyStart = 0), (this.dirtyEnd = 0);
    const { pixelType: i, layout: s, textureOnly: n } = t;
    (this.textureOnly = n || !1),
      ...
      (this.size = e),
      (this.isLocal = r),
      n || (this.data = this._initData(i, e));
  }
  get buffer() {
    var e;
    return (e = this.data) == null ? void 0 : e.buffer;
  }
  ...
  setData(e, t, r) {
    ...
    this.layout & s
      ? this.data != null &&
        ((this.data[i * this.texelSize + t] = r),
  ...
  expand(e) {
    if (((this.size = e), !this.textureOnly)) {
      const t = this._initData(this.pixelType, e),
        r = this.data;
      t.set(r), (this.data = t);
    }
  }
  ...
  _initData(e, t) {
    const r = ArrayBuffer,
      i = ce(e),
      s = new i(new r(t * t * 4 * i.BYTES_PER_ELEMENT));
    for (let n = 0; n < s.length; n += 4) s[n + 1] = 255;
    return s;
  }
  _resetRange() {
...
class Wt {
  constructor(e) {
    (this._client = e),
      ...
      (this._size = Ce),
      (this._idsToHighlight = new Map()),
      (this._arcadeDependencies = new Set()),
      (this._initialized = !1),
      (this.version = 0),
      (this._idGenerator = new tt()),
      (this._epoch = 1);
  }
  destroy() {
  ...
  _initialize() {
    ...
    const e = T.FLOAT;
    L(`Creating AttributeStore ${gt.sharedArrayBuffer ? 'with' : 'without'} shared memory`),
      (this._blockDescriptors = [
        { pixelType: T.UNSIGNED_BYTE, layout: 1 },
        { pixelType: T.UNSIGNED_BYTE, layout: 15, textureOnly: !0 },
        { pixelType: T.UNSIGNED_BYTE, layout: 15, textureOnly: !0 },
        { pixelType: e, layout: 15 },
        { pixelType: e, layout: 15 },
        { pixelType: e, layout: 15 },
        { pixelType: e, layout: 15 },
        { pixelType: T.FLOAT, layout: 15 },
      ]),
  ...
  async update(e, t, r, i = 0) {
    const s = Ee(this._schema, e);
    if (
      ((this.version = i),
      s &&
        (m('esri-2d-update-debug') &&
          console.debug(`Version[${i}] AttributeStore.update`, { changed: s }),
        (this._schema = e),
        (this._attributeComputeInfo = null),
        this._initialize(),
        e != null))
      ...
      if (
        (r &&
          (this._filters = await Promise.all(
            e.filters.map((n) =>
              n
                ? yt.create({
                    ...
                    hasZ: !1,
                    timeInfo: r.timeInfo,
                    fieldsIndex: r.fieldsIndex,
                    spatialReference: r.outSpatialReference,
                    filterJSON: n,
                  })
                : null,
            ),
        ...
        (this._attributeComputeInfo = { type: 'feature', map: new Map() }),
          await Promise.all(
            e.bindings.map(async (n) => {
              const o = await this._bind(t, n);
              this._updateReferences(o);
            }),
          );
      else {
        this._attributeComputeInfo = { type: 'multi', keyField: e.keyField, map: new Map() };
        for (const n in e.bindings) {
          const o = e.bindings[n];
          await Promise.all(
            o.map(async (h) => {
              const u = await this._bind(t, h, parseInt(n, 10));
              this._updateReferences(u);
            }),
          );
        }
      }
  ...
  setHighlight(e, t) {
    ...
    r.unsetComponentAllTexels(0, (1 << k) - 1);
    for (const { displayId: i, highlightFlags: s } of e) {
      if (i == null) continue;
      const n = w(i);
      r.setComponent(0, s, [n]);
    }
  ...
  setAttributeData(e, t, r, i) {
    const s = w(e);
    this._ensureSizeForTexel(s), this._getBlock(0).setData(s, 0, this.getFilterFlags(t, i));
    const n = this._attributeComputeInfo,
      o = 1,
      h = 4;
    let u = null;
    n &&
      ((u = n.type === 'multi' ? n.map.get(t.readAttribute(n.keyField)) : n.map),
      u != null &&
        u.size &&
        u.forEach((l, f) => {
          var c;
          const y = (f * o) % h,
            I = Math.floor((f * o) / h),
            S = this._getBlock(I + v.VV);
          let p = (c = l.field) == null ? void 0 : c.read(t, r);
          l.valueRepresentation && (p = mt(p, l.valueRepresentation)),
            (p === null || isNaN(p) || p === 1 / 0 || p === -1 / 0) && (p = Oe),
            S.setData(s, y, p);
        }));
  }
  get epoch() {
  ...
  sendUpdates() {
    const e = this._blocks.map((r) => (r != null ? r.toMessage() : null)),
      t = this._getInitArgs();
    m('esri-2d-log-updating') && console.log('AttributeStore: _doSendUpdate.start'),
      this._client.update({
        initArgs: t,
        blockData: e,
      ...
      (this._epoch += 1),
      m('esri-2d-log-updating') && console.log('AttributeStore: _doSendUpdate.end');
  }
  _ensureSizeForTexel(e) {
    for (; e >= this._size * this._size; ) if (this._expand()) return;
  }
  async _bind(e, t, r) {
    const i = await e.createComputedField(t),
      { valueRepresentation: s } = t,
      n = this._attributeComputeInfo;
    if (n.type === 'multi') {
      const o = n.map.get(r) ?? new Map();
      o.set(t.binding, { field: i, valueRepresentation: s }), n.map.set(r, o);
    } else n.map.set(t.binding, { field: i, valueRepresentation: s });
    return i;
  }
  _getInitArgs() {
    return this._initialized
      ? null
      : ((this._initialized = !0),
        this._getBlock(v.Animation),
        this._getBlock(v.GPGPU),
        this._getBlock(v.LocalTimeOrigin),
        {
          blockSize: this._size,
          blockDescriptors: this._blocks.map((e) =>
            e != null
              ? { textureOnly: e.textureOnly, buffer: e.buffer, pixelType: e.pixelType }
              : null,
          ),
        });
  }
  _getBlock(e) {
    ...
    if (t != null) return t;
    L(`Initializing AttributeBlock at index ${e}`);
    const r = new bt(this._size, this._blockDescriptors[e], this._client.isLocal);
    return (this._blocks[e] = r), (this._initialized = !1), r;
  }
  _expand() {
    if (this._size < this._schema.capabilities.maxTextureSize) {
      const e = (this._size <<= 1);
      L('Expanding block size to', e, this._blocks);
      for (const t of this._blocks) t == null || t.expand(e);
      return (this._initialized = !1), (this._size = e), 0;
    }
    return (
      Z().error(new b('mapview-limitations', 'Maximum number of onscreen features exceeded.')), -1
    );
  }
  _updateReferences(e) {
    De(this._arcadeDependencies, e);
  }
  isVisible(e) {
    return !!(this._getBlock(0).getData(e, 0) & (1 << k));
  }
  getFilterFlags(e, t) {
    let r = 0;
    for (let s = 0; s < this._filters.length; s++) {
      const n = !!(1 << s),
        o = this._filters[s];
      r |= (!n || o == null || o.check(e, t) ? 1 : 0) << s;
    }
    let i = 0;
    if (this._idsToHighlight.size) {
      const s = e.getObjectId();
      i = this.getHighlightFlags(s);
    }
    return (r << k) | i;
  }
}
...
function q(a, e, t) {
  ...
  e.contextTimeZone = (i = t.$view) == null ? void 0 : i.timeZone;
  try {
    return a.evaluate({ ...t, $feature: r }, a.services);
  } catch (s) {
    return (
      A.getLogger('esri.views.2d.support.arcadeOnDemand').warn(
...
function oe(a) {
  return a == null || a === 1 / 0 || a === -1 / 0 || (typeof a == 'number' && isNaN(a));
}
function ee(a, e, t, r) {
  var n;
  if (a == null) return r ?? null;
  const i = e.readArcadeFeature();
  e.contextTimeZone = (n = t.$view) == null ? void 0 : n.timeZone;
  const s = a.evaluate({ ...t, $feature: i }, a.services);
  return oe(s) ? (r ?? null) : s;
}
function xt(a, e) {
  const t = 43758.5453 * Math.sin(12.9898 * a + 78.233 * e);
  return t - Math.floor(t);
}
let wt = class he extends $ {
    static async create(e, t) {
      const r = await ve(e, t.spatialReference, t.fields),
        i = re(e);
      return new he(r, i);
    }
    constructor(e, t) {
    ...
    resize(e) {}
    read(e, t) {
      return this.hasArcadeDependency('scale') || t.$view.timeZone !== 'system'
        ? q(this._compiled, e, t)
        : this._readCached(e, t);
    }
    readWithDefault(e, t, r) {
      return this.hasArcadeDependency('scale') || t.$view.timeZone !== 'system'
        ? ee(this._compiled, e, t, r)
        : this._readWithDefaultCached(e, t, r);
    }
    hasArcadeDependency(e) {
      var t;
      return ((t = this._compiled) == null ? void 0 : t.references(e)) ?? !1;
    }
    _getCacheKey(e) {
      var i;
      if (!((i = this._compiled) != null && i.references('timeProperties'))) return this._cacheKey;
      const { currentStart: t, currentEnd: r } = e.$view.timeProperties;
      return this._cacheKey + xt(t ?? 1, r ?? 1);
    }
    _readCached(e, t) {
    ...
    _readWithDefaultCached(e, t, r) {
      if ((e.setCache(this._getCacheKey(t)), e.hasCachedValue())) return e.getCachedValue();
      const i = ee(this._compiled, e, t, r);
  ...
  St = class ue extends $ {
    static async create(e, t) {
      ...
      return new ue((i) =>
        r.replaceAll(/{[^}]*}/g, (s) => {
          const n = s.slice(1, -1),
            o = i.metadata.fieldsIndex.get(n);
          if (o == null) return s;
          const h = i.readAttribute(n);
          return h == null ? '' : Pe(h, o);
        }),
      );
    }
    constructor(e) {
...
const te = () => A.getLogger('esri.views.2d.layers.features.support.ComputedAttributeStorage'),
  x = 4294967295;
function z(a, e, t) {
  if (!(a.length > e)) for (; a.length <= e; ) a.push(t);
}
class Pt {
  constructor(e) {
    (this._numerics = []),
      ...
      (this._instanceIds = []),
      (this._bounds = []),
      (this._dirtyBitset = this.getBitset(this.createBitset())),
      (this.compilationOptions = e);
  }
  createBitset() {
    const e = this._bitsets.length;
    return this._bitsets.push(se.create(this._allocatedSize, _)), e + 1;
  }
  async createComputedField(e, t = !1) {
    if (e.expression)
      try {
        ...
        return t
          ? await St.create(e.expression, this.compilationOptions)
          : await wt.create(e.expression, this.compilationOptions);
      } catch (i) {
        const s = new b('featurelayer', 'Failed to compile arcade expression', {
          error: i,
          expression: e.expression,
        });
        return te().error(s), null;
      }
    if (e.normalizationType || e.normalizationField) return new Tt(e.field, e);
    if (e.field) return new It(e.field);
    const r = new b(
      'featurelayer',
      'Unable to create computed field. No expression or field found',
      { info: e },
    );
    return te().error(r), null;
  }
  async createWhereClause(e) {
    return e ? ne(e, this.compilationOptions.fields) : null;
  }
  getBitset(e) {
    return this._bitsets[e - 1];
  }
  getComputedNumeric(e, t) {
    return this.getComputedNumericAtIndex(e & _, 0);
  }
  setComputedNumeric(e, t, r) {
    return this.setComputedNumericAtIndex(e & _, r, 0);
  }
  getComputedString(e, t) {
    return this.getComputedStringAtIndex(e & _, 0);
  }
  setComputedString(e, t, r) {
    return this.setComputedStringAtIndex(e & _, 0, r);
  }
  ...
  getXMin(e) {
    return this._bounds[4 * (e & _)];
  }
  getYMin(e) {
    return this._bounds[4 * (e & _) + 1];
  }
  getXMax(e) {
    return this._bounds[4 * (e & _) + 2];
  }
  getYMax(e) {
    return this._bounds[4 * (e & _) + 3];
  }
  setBounds(e, t, r = !1) {
    const i = e & _;
    if (!r && !this._dirtyBitset.has(e)) return this._bounds[4 * i] !== x;
    this._dirtyBitset.unset(e);
    const s = t.readGeometryWorldSpace();
    if ((z(this._bounds, 4 * i + 4, 0), !s || !s.coords.length))
      return (
        (this._bounds[4 * i] = x),
        (this._bounds[4 * i + 1] = x),
        (this._bounds[4 * i + 2] = x),
        (this._bounds[4 * i + 3] = x),
        !1
      );
    let n = 1 / 0,
      o = 1 / 0,
    ...
    return (
      s.forEachVertex((l, f) => {
        (n = Math.min(n, l)), (o = Math.min(o, f)), (h = Math.max(h, l)), (u = Math.max(u, f));
      }),
      (this._bounds[4 * i] = n),
      (this._bounds[4 * i + 1] = o),
      (this._bounds[4 * i + 2] = h),
      (this._bounds[4 * i + 3] = u),
      !0
    );
  }
  getBounds(e, t) {
    const r = this.getXMin(t),
      i = this.getYMin(t),
      s = this.getXMax(t),
      n = this.getYMax(t);
    return W(e, r, i, s, n), r !== x;
  }
  _ensureNumeric(e, t) {
    this._numerics[e] || (this._numerics[e] = []), z(this._numerics[e], t, 0);
  }
  _ensureInstanceId(e) {
    z(this._instanceIds, e, 0);
  }
  _ensureString(e, t) {
    this._strings[e] || (this._strings[e] = []), z(this._strings[e], t, null);
  }
}
class F {
  ...
  constructor(e) {
    (this._options = e),
      (this._fieldsIndex = M.fromJSON(e.fieldsIndex)),
      e.spatialReference && (this._spatialReference = J.fromJSON(e.spatialReference)),
      e.outSpatialReference && (this._outSpatialReference = J.fromJSON(e.outSpatialReference)),
      (this._arcadeSchema = {
  ...
  get geometryType() {
    return this._options.geometryType === 'esriGeometryMultiPatch'
      ? 'esriGeometryPolygon'
      : this._options.geometryType;
  }
  ...
  weakCloneWithAdditionalFields(e) {
    return new F({
      fieldsIndex: { fields: [...this._fieldsIndex.fields, ...e], timeZoneByFieldName: null },
      geometryType: this.geometryType,
