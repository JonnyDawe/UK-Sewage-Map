...
class y {
  constructor(e) {
    (this._schedule = null),
      (this._deferreds = new o()),
      (this._controllers = new o()),
      (this._processingItems = new o()),
      (this._pausedSignal = p(!1)),
      (this.concurrency = 1),
      e.concurrency && (this.concurrency = e.concurrency),
      (this._queue = new d(e.peeker)),
      (this.process = e.process);
  }
  destroy() {
    this.clear(), (this._schedule = g(this._schedule));
  }
  get updating() {
    return this.running;
  }
  ...
  clear() {
    this._queue.clear();
    const e = [];
    this._controllers.forEach((s) => e.push(s)),
  ...
  forEach(e) {
    this._deferreds.forEach((s, i) => e(i));
  }
  get(e) {
  ...
  push(e, s) {
    ...
    s && (r = m(s, () => t.abort()));
    const _ = () => {
        const n = this._processingItems.get(e);
        n && n.controller.abort(), c(), h.reject(l());
      },
      c = () => {
        a.remove(),
          r != null && r.remove(),
  ...
  popLast() {
    ...
    return e && ((s = this._deferreds.get(e)) == null || s.reject(l()), this._removeItem(e)), e;
  }
  reset() {
  ...
  _processResult(e, s) {
    this._canProcessFulfillment(e) &&
      (this._scheduleNext(), this._deferreds.get(e.item).resolve(s));
  }
  _processError(e, s) {
  ...
  _process(e) {
    ...
    const i = new AbortController(),
      t = new x(e, i);
    this._processingItems.set(e, t);
    try {
      s = this.process(e, i.signal);
    } catch (r) {
      this._processError(t, r);
    }
    q(s)
      ? ((t.promise = s),
        s.then(
          (r) => this._processResult(t, r),
          (r) => this._processError(t, r),
        ))
      : this._processResult(t, s);
  }
}
export { y as QueueProcessor };
