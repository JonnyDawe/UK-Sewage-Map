...
import { t as L } from './VertexElementDescriptor-BLyltQyJ.js';
const I = () => N.getLogger('esri.views.2d.engine.webgl.Utils');
function v(r) {
  switch (r) {
    ...
    case h.FLOAT:
      return 4;
    default:
      return void I().error(new p('webgl-utils', `Unable to handle type ${r}`));
  }
}
function D(r) {
  switch (r) {
...
function b(r) {
  ...
  for (const e in r) {
    ...
    t.set(
      e,
      n.map((i) => {
        const a = new L(i.name, i.count, i.type, s, 0, i.normalized || !1);
        return (s += i.count * A(i.type)), a;
      }),
    ),
...
const B = (r) => {
  ...
  R = (r) => {
    const t = {};
    return r.forEach((e, n) => (t[n] = e != null && e.length ? e[0].stride : 0)), t;
  },
  E = new Map(),
  G = (r, t) => {
    if (!E.has(r)) {
      const e = b(t),
        n = { strides: R(e), bufferLayouts: e, attributes: B(t) };
      E.set(r, n);
    }
    return E.get(r);
  },
  X = (r) => r.includes('data:image/svg+xml');
function W(r) {
  const t = [];
...
function K(r) {
  ...
  switch (t) {
    case 'CIMMarkerPlacementAlongLineRandomSize':
      return `${t}-${r.seed}-${r.randomization}`;
    case 'CIMMarkerPlacementAlongLineVariableSize':
      return `${t}-${r.maxRandomOffset}-${r.numberOfSizes}-${r.seed}-${r.variationMethod}`;
    case 'CIMMarkerPlacementAtExtremities':
      return `${t}-${r.extremityPlacement}-${r.offsetAlongLine}`;
    case 'CIMMarkerPlacementAtRatioPositions':
      return `${t}-${r.beginPosition}-${r.endPosition}-${r.flipFirst}-${JSON.stringify(r.positionArray)}`;
    case 'CIMMarkerPlacementAtMeasuredUnits':
      return `${t}-${r.interval}-${r.skipMarkerRate}-${r.placeAtExtremities}`;
    case 'CIMMarkerPlacementInsidePolygon':
      return `${t}-${r.stepX}-${r.stepY}-${r.randomness}-${r.gridType}-${r.seed}-${r.shiftOddRows}`;
    case 'CIMMarkerPlacementOnLine':
      return `${t}-${r.relativeTo}-${r.startPointOffset}`;
    case 'CIMMarkerPlacementOnVertices':
      return `${t}-${r.placeOnControlPoints}-${r.placeOnEndPoints}-${r.placeOnRegularVertices}`;
...
let T = class d {
  ...
  static write(t, e, n, s, i, a, c, u) {
    t.push(e), t.push(n), t.push(s), t.push(i), t.push(a), t.push(c), t.push(u);
  }
  serialize(t) {
    return (
      t.push(this.instanceId),
      t.push(this.textureKey),
      t.push(this.indexStart),
      t.push(this.indexCount),
      t.push(this.vertexStart),
      t.push(this.vertexCount),
      t.push(this.overlaps),
      t
    );
  }
...
let $ = class M {
  ...
  serialize(t) {
    return t.push(this.id), t.writeF32(this.sortKey), m(t, this.records), t;
  }
  static deserialize(t) {
    const e = t.readInt32(),
      n = t.readF32(),
      s = new M(e, n);
    return (s.records = S(t, T) ?? []), s;
  }
};
$.byteSizeHint = 2 * Uint32Array.BYTES_PER_ELEMENT + T.byteSizeHint;
class f {
  constructor(t, e, n, s) {
    (this.transformedX = 0),
      (this.transformedY = 0),
  ...
  findCollisionDelta(t, e = 4) {
    const n = Math.abs(t.centerT[0] - this.centerT[0]),
      s = Math.abs(t.centerT[1] - this.centerT[1]),
      i = (t.halfWidth + this.halfWidth + e) / n,
      a = (t.halfHeight + this.halfHeight + e) / s,
      c = Math.min(i, a);
    return Math.log2(c);
  }
  extend(t) {
    const e = Math.min(this.xmin, t.xmin),
      n = Math.min(this.ymin, t.ymin),
      s = Math.max(this.xmax, t.xmax) - e,
      i = Math.max(this.ymax, t.ymax) - n,
      a = e + s / 2,
      c = n + i / 2;
    (this.width = s),
      (this.height = i),
  ...
  static deserialize(t) {
    const e = t.readF32(),
      ...
      s = t.readInt32(),
      i = t.readInt32();
    return new f(e, n, s, i);
  }
}
class _ {
  constructor(t, e, n, s, i, a, c, u, l = []) {
    (this.entityTexel = t),
      (this.anchorX = e),
      (this.anchorY = n),
      (this.directionX = s),
      ...
      (this.maxScale = a),
      (this.minScale = c),
      (this.referenceBounds = u),
      (this.bounds = l);
  }
  serialize(t) {
    t.push(this.entityTexel),
      t.writeF32(this.anchorX),
      t.writeF32(this.anchorY),
      t.writeF32(this.directionX),
      t.writeF32(this.directionY),
      t.writeF32(this.maxScale),
      t.writeF32(this.minScale),
      this.referenceBounds === null
        ? (t.writeF32(0), t.writeF32(0), t.writeF32(0))
        : (t.writeF32(this.referenceBounds.size),
          t.writeF32(this.referenceBounds.offsetX),
          t.writeF32(this.referenceBounds.offsetY)),
      m(t, this.bounds);
  }
  static deserialize(t) {
    const e = t.readInt32(),
      ...
      w = t.readF32(),
      U = S(t, f) ?? [];
    return new _(e, n, s, i, a, c, u, { size: l, offsetX: F, offsetY: w }, U);
  }
}
const x = new Float32Array(1),
  O = new Uint32Array(x.buffer);
function z(r) {
  return (x[0] = r), O[0];
}
function V(r, t) {
...
function J(r, t, e, n) {
  ...
  switch (e.type) {
    ...
    case o.FLOAT:
      ...
      break;
    case o.HALF_FLOAT:
      if (e.count === 1) r.setUint16(n + e.offset, g(t * s), !0);
      else
        for (let i = 0; i < e.count; i++) {
          const a = i * Uint16Array.BYTES_PER_ELEMENT;
          r.setUint16(n + e.offset + a, g(t[i] * s), !0);
        }
  }
}
function Z(r, t, e) {
  switch (t.type) {
    ...
    case o.HALF_FLOAT: {
      if (t.count === 1) return y(r.getUint16(e + t.offset, !0));
      const n = [];
      for (let s = 0; s < t.count; s++) {
        const i = s * Uint16Array.BYTES_PER_ELEMENT;
        n.push(y(r.getUint16(e + t.offset + i, !0)));
      }
      return n;
    }
  }
