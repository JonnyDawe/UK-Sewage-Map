const __vite__mapDeps = (
  ...
  d = m.f ||
    (m.f = [
      'assets/streamLayerUtils-BP-qrfEI.js',
      ...
      'assets/createConnection-zAolrfez.js',
    ]),
) => i.map((i) => d[i]);
import { _ as f } from './index-DqxZnyqH.js';
import {
  aj as P,
  A as t,
  B as s,
...
let i = class extends _(A(E(O(k(D(U(C(V(N(L(G(J(M.ClonableMixin(q)))))))))))))) {
    ...
    super(...e),
      ...
      (this._debouncedSaveOperations = z(async (r, o, a) => {
        const { save: n, saveAs: l } = await f(
          () => import('./streamLayerUtils-BP-qrfEI.js'),
  ...
  async connect(e) {
    const [{ createConnection: r }] = await Promise.all([
        f(() => import('./createConnection-zAolrfez.js'), __vite__mapDeps([9, 3, 4, 2, 5])),
        this.load(),
      ]),
