import { H as c, a7 as h } from './Theme-C50qTSEX.js';
class u extends c {
  _afterNew() {
  ...
  generateColors() {
    ...
    const e = this.getPrivate('currentPass'),
      t = this.get('colors', [this.get('baseColor', h.fromHex(16711680))]);
    this.getPrivate('numColors') || this.setPrivate('numColors', t.length);
    const r = this.getPrivate('numColors'),
      g = 0,
      i = this.get('passOptions'),
      P = this.get('reuse');
    for (let a = g; a < r; a++)
      if (P) t.push(t[a]);
      else {
