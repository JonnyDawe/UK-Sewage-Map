...
import { t as y } from './key-ak3_eHKH.js';
  ...
  s = class s extends v {
    constructor() {
      super(),
        ...
        (this.calciteChipClose = a({ cancelable: !1 })),
        (this.calciteChipSelect = a({ cancelable: !1 })),
        (this.calciteInternalChipKeyEvent = a({ cancelable: !1 })),
        (this.calciteInternalChipSelect = a({ cancelable: !1 })),
        (this.calciteInternalSyncSelectedChips = a({ cancelable: !1 })),
    ...
    async setFocus() {
      var e, l;
      await m(this),
        !this.disabled && this.interactive
          ? (e = this.containerEl.value) == null || e.focus()
          : !this.disabled &&
            this.closable &&
            ((l = this.closeButtonEl.value) == null || l.focus());
    }
    async load() {
      b() && this.updateHasText();
    }
    willUpdate(e) {
    ...
    handleSelectionPropertyChange(e) {
      this.selectionMode === 'single' && this.calciteInternalSyncSelectedChips.emit(),
        !this.parentChipGroup.selectedItems.includes(this.el) &&
          e &&
    ...
    renderChipImage() {
      return t`<div class=${i(c.imageContainer)}><slot name=${$.image} @slotchange=${this.handleSlotImageChange}></slot></div>`;
    }
    renderSelectionIcon() {
      const e =
        this.selectionMode === 'multiple'
          ...
          : this.selected
            ? n.checkedSingle
            : void 0;
      return t`<div class=${i({ [c.selectIcon]: !0, [c.selectIconActive]: this.selectionMode === 'multiple' || this.selected })}>${e ? t`<calcite-icon .icon=${e} .scale=${r(this.scale)}></calcite-icon>` : null}</div>`;
    }
    renderCloseButton() {
      return t`<button .ariaLabel=${this.messages.dismissLabel} class=${i(c.close)} @click=${this.close} @keydown=${this.closeButtonKeyDownHandler} .tabIndex=${this.disabled ? -1 : 0} ${h(this.closeButtonEl)}><calcite-icon .icon=${n.close} .scale=${r(this.scale)}></calcite-icon></button>`;
    }
    renderIcon() {
      return t`<calcite-icon class=${i(c.chipIcon)} .flipRtl=${this.iconFlipRtl} .icon=${this.icon} .scale=${r(this.scale)}></calcite-icon>`;
    }
    render() {
      const { disabled: e } = this,
        l = e || (!e && !this.interactive),
        d =
      ...
      return k({
        disabled: e,
        children: t`<div .ariaChecked=${this.selectionMode !== 'none' && this.interactive ? this.selected : void 0} .ariaLabel=${this.label} class=${i({ [c.container]: !0, [c.textSlotted]: this.hasText, [c.imageSlotted]: this.hasImage, [c.selectable]: this.selectionMode !== 'none', [c.multiple]: this.selectionMode === 'multiple', [c.single]: this.selectionMode === 'single' || this.selectionMode === 'single-persist', [c.selected]: this.selected, [c.closable]: this.closable, [c.nonInteractive]: !this.interactive, [c.isCircle]: !this.closable && !this.hasText && (!this.icon || !this.hasImage) && (this.selectionMode === 'none' || (!!this.selectionMode && this.selectionMode !== 'multiple' && !this.selected)) })} @click=${this.handleEmittingEvent} .role=${d} .tabIndex=${l ? -1 : 0} ${h(this.containerEl)}>${(this.selectionMode !== 'none' && this.renderSelectionIcon()) || ''}${this.renderChipImage()}${(this.icon && this.renderIcon()) || ''}<span class=${i(c.title)}><slot @slotchange=${this.handleDefaultSlotChange}></slot></span>${(this.closable && this.renderCloseButton()) || ''}</div>`,
      });
    }
  };
(s.properties = {
