...
import { at as Q, C as He, au as Ye, B as et, av as tt } from './arcade-Cy3NLj9n.js';
import {
  S as L,
  t as p,
  g as j,
  ...
  d as g,
  l as ut,
} from './SpatialFilter-DweRxrGR.js';
import { D, n as x, a as N, i as dt, u as ht } from './WhereClause-DynWr3o5.js';
let Ne = class {
  constructor() {
...
class B extends L {
  constructor(e) {
    super(e),
      ...
      e.whereclause instanceof D
        ? ((this._whereclause = e.whereclause), (this._whereClauseFunction = null))
        : ((this._whereClauseFunction = e.whereclause), (this._whereclause = null));
  }
  _initialiseFeatureSet() {
    this._parent !== null
      ? ((this.fields = this._parent.fields.slice()),
        ...
        (this.subtypeField = this._parent.subtypeField),
        (this.subtypes = this._parent.subtypes))
      : ((this.fields = []),
        (this.typeIdField = ''),
        (this.subtypeField = ''),
        (this.objectIdField = ''),
        (this.globalIdField = ''),
        (this.spatialReference = new ae({ wkid: 4326 })),
        (this.geometryType = ne.point));
  }
  async _getSet(e) {
    if (this._wset === null) {
      ...
      return (
        ...
        this._whereClauseFunction !== null
          ? (this._wset = new p(
              t._candidates.slice().concat(t._known.slice()),
              [],
              t._ordered,
  ...
  _isInFeatureSet(e) {
    var s;
    let t = (s = this._parent) == null ? void 0 : s._isInFeatureSet(e);
  ...
  async _fetchAndRefineFeatures(e, t, s) {
    var n, l, u;
    const i = new p([], e, !1, null),
      r = Math.min(t, e.length);
    if (
      (await ((n = this._parent) == null ? void 0 : n._getFeatures(i, -1, r, s)),
      ...
      for (let o = 0; o < r; o++) {
        ...
        this.executeWhereClause(d) === !0
          ? (this._idstates[e[o]] = S.InFeatureSet)
          : (this._idstates[e[o]] = S.NotInFeatureSet);
      }
      return 'success';
    }
    const a = [];
  ...
  async _getFilteredSet(e, t, s, i, r) {
    this._whereClauseFunction !== null ||
      (s !== null
        ? this._whereclause !== null && (s = j(this._whereclause, s))
        : (s = this._whereclause)),
      await this._ensureLoaded();
    ...
    return (
      ...
      (n =
        this._whereClauseFunction !== null
          ? new p(
              a._candidates.slice().concat(a._known.slice()),
              [],
              a._ordered,
  ...
  async _stat(e, t, s, i, r, a, n) {
    if (this._whereClauseFunction !== null)
      return r === null && s === '' && i === null
        ? this._manualStat(e, t, a, n)
        : { calculated: !1 };
    let l = this._whereclause;
    r !== null && this._whereclause !== null && (l = j(this._whereclause, r));
  ...
  async _canDoAggregates(e, t, s, i, r) {
    return (
      this._whereClauseFunction === null &&
      (r !== null
        ? this._whereclause !== null && (r = j(this._whereclause, r))
        : (r = this._whereclause),
      this._parent !== null && this._parent._canDoAggregates(e, t, s, i, r))
    );
  }
  async _getAggregatePagesDataSourceDefinition(e, t, s, i, r, a, n) {
    if (this._parent === null) throw new C(T.NeverReach);
    return (
      r !== null
        ? this._whereclause !== null && (r = j(this._whereclause, r))
        : (r = this._whereclause),
      this._parent._getAggregatePagesDataSourceDefinition(e, t, s, i, r, a, n)
    );
  }
  ...
  ct = class extends X {
    ...
    rewriteSql(e, t) {
      return {
        ...
        where: V(e, this.field.name, this.originalField.name, t.getFieldsIndex()),
      };
    }
    extractValue(e) {
...
class w extends X {
  ...
  rewriteSql(e, t) {
    const s = this.evaluateNodeToWhereClause(
      e.parseTree,
      k.Standardised,
      this.field.name,
      this.codefield instanceof D ? R(this.codefield, k.Standardised) : this.codefield,
      e.parameters,
    );
    return s.includes(w.BADNESS)
      ? { rewritten: !1, where: e }
      : {
          rewritten: this.sqlRewritable,
          where: D.create(s, {
  ...
  evaluateNodeToWhereClause(e, t, s = null, i = null, r) {
    ...
    switch (e.type) {
      case 'interval':
        return at(this.evaluateNodeToWhereClause(e.value, t, s, i, r), e.qualifier, e.op);
      case 'case-expression': {
        let o = ' CASE ';
        e.format === 'simple' &&
          (o += this.evaluateNodeToWhereClause(e.operand, t, s, w.BADNESS, r));
        for (let d = 0; d < e.clauses.length; d++)
          o +=
            ' WHEN ' +
            this.evaluateNodeToWhereClause(e.clauses[d].operand, t, s, w.BADNESS, r) +
            ' THEN ' +
            this.evaluateNodeToWhereClause(e.clauses[d].value, t, s, w.BADNESS, r);
        return (
          e.else !== null &&
            (o += ' ELSE ' + this.evaluateNodeToWhereClause(e.else, t, s, w.BADNESS, r)),
          (o += ' END '),
          o
        );
      }
      case 'parameter': {
        ...
        if (fe(o)) return Y(o, t);
        if (pe(o)) return Y(o, t);
        if (ye(o)) return Ie(o, t);
        if (_e(o)) return be(o, t);
        if (ge(o)) return Ce(o, t);
        if (Array.isArray(o)) {
          const d = [];
          for (let c = 0; c < o.length; c++)
            typeof o[c] == 'string'
              ? d.push("'" + o[c].toString().replaceAll("'", "''") + "'")
              : fe(o[c]) || pe(o[c])
                ? d.push(Y(o[c], t))
                : ye(o[c])
                  ? d.push(Ie(o[c], t))
      ...
      case 'unary-expression':
        return ' ( NOT ' + this.evaluateNodeToWhereClause(e.expr, t, s, w.BADNESS, r) + ' ) ';
      case 'binary-expression':
        switch (e.operator) {
          case 'AND':
            return (
              ' (' +
              this.evaluateNodeToWhereClause(e.left, t, s, i, r) +
              ' AND ' +
              this.evaluateNodeToWhereClause(e.right, t, s, i, r) +
              ') '
            );
          case 'OR':
            return (
          ...
          case 'IN':
            if (((a = []), e.right.type === 'expression-list')) {
              ...
              return (
                (a = this.evaluateNodeToWhereClause(e.right, t, s, i, r)),
                ' (' +
                  this.evaluateNodeToWhereClause(e.left, t, s, i, r) +
                  ' IN (' +
                  a.join(',') +
            ...
            return (
              (u = this.evaluateNodeToWhereClause(e.right, t, s, i, r)),
              Array.isArray(u)
                ? ' (' +
                  this.evaluateNodeToWhereClause(e.left, t, s, i, r) +
                  ' IN (' +
                  u.join(',') +
                  ')) '
                : ' (' + this.evaluateNodeToWhereClause(e.left, t, s, i, r) + ' IN (' + u + ')) '
            );
          case 'NOT IN':
            if (((a = []), e.right.type === 'expression-list')) {
              if (
          ...
          case 'BETWEEN':
            return (
              (l = this.evaluateNodeToWhereClause(e.right, t, s, w.BADNESS, r)),
              ' (' +
                this.evaluateNodeToWhereClause(e.left, t, s, w.BADNESS, r) +
                ' BETWEEN ' +
                l[0] +
                ' AND ' +
                l[1] +
          ...
          case 'LIKE':
            return e.escape !== ''
              ? ' (' +
                  this.evaluateNodeToWhereClause(e.left, t, s, w.BADNESS, r) +
                  ' LIKE ' +
                  this.evaluateNodeToWhereClause(e.right, t, s, w.BADNESS, r) +
                  " ESCAPE '" +
                  e.escape +
                  "') "
              : ' (' +
      ...
      case 'column-reference':
        return s && s.toLowerCase() === e.column.toLowerCase() ? '(' + i + ')' : it(e.column);
      case 'data-type':
        return e.value;
      case 'function': {
        const o = this.evaluateNodeToWhereClause(e.args, t, s, w.BADNESS, r);
        return st(e.name, o, t);
      }
    }
    throw new x(N.UnsupportedSyntax, { node: e.type });
  }
  extractValue(e) {
    return this.codefield instanceof D
      ? this.reverseLkp[D.convertValueToStorageFormat(this.codefield.calculateValueCompiled(e))]
      : this.reverseLkp[e.attributes[this.codefield]];
  }
}
w.BADNESS = '_!!!_BAD_LKP_!!!!';
class ft extends X {
  constructor(e, t) {
    super(e), (this._sql = t);
  }
  rewriteSql(e, t) {
    return {
      rewritten: !0,
      where: V(e, this.field.name, R(this._sql, k.Standardised), t.getFieldsIndex()),
    };
  }
  extractValue(e) {
...
let pt = class extends L {
    ...
    if (this._wset === null) {
      ...
      return (
        ...
        (this._wset = new p(
          ...
          s._known.slice(),
          s._ordered,
          this._clonePageDefinition(s.pagesDefinition),
        )),
        this._wset
      );
    }
  ...
  async _getFeatures(e, t, s, i) {
    ...
    if (this._checkIfNeedToExpandKnownPage(e, a) === !0)
      return await this._expandPagedSet(e, a, 0, 0, i), this._getFeatures(e, t, s, i);
    let n = 0;
    for (
      let c = e._lastFetchedIndex;
      c < e._known.length &&
      (n++,
      n <= s && (e._lastFetchedIndex += 1),
      !(
        this._featureCache[e._known[c]] === void 0 &&
        (e._known[c] !== t && r.push(e._known[c]), r.length >= a)
      ));
      c++
    );
    if (r.length === 0) return 'success';
    e = new p([], r, e._ordered, null);
    const l = Math.min(r.length, s);
    await ((o = this._parent) == null ? void 0 : o._getFeatures(e, -1, l, i)),
      this._checkCancelled(i);
    const u = [];
    for (let c = 0; c < l; c++) {
      const f = (d = this._parent) == null ? void 0 : d._featureFromCache(r[c]);
      f !== void 0 && u.push({ geometry: f.geometry, attributes: f.attributes, id: r[c] });
    }
    for (const c of u) {
      const f = [];
      for (const y of this.adaptedFields) f[y.field.name] = y.extractValue(c);
      this._featureCache[c.id] = new q({ attributes: f, geometry: qe(c.geometry) });
    }
    return 'success';
  }
  async _fetchAndRefineFeatures() {
    throw new C(T.NeverReach);
  }
  async _getFilteredSet(e, t, s, i, r) {
    let a = !1;
    ...
    if (i !== null) {
      ...
      for (const c of this.adaptedFields)
        if (!(c instanceof se) && i.scanForField(c.field.name) === !0) {
          if (!(c instanceof ct)) {
            (i = null), (l = !1);
            break;
          }
      ...
      i && d.length > 0 && (i = i.replaceFields(d));
    }
    s !== null
      ? this._extraFilter !== null && (s = j(this._extraFilter, s))
      : (s = this._extraFilter),
      await this._ensureLoaded();
    ...
    return (
      ...
      (o =
        a === !0
          ? new p(
              u._candidates.slice().concat(u._known.slice()),
              [],
              l === !0 && u._ordered,
              this._clonePageDefinition(u.pagesDefinition),
            )
          : new p(
              u._candidates.slice(),
              u._known.slice(),
  ...
  _reformulateWithoutAdaptions(e) {
    ...
    if (e !== null) {
      for (const s of this.adaptedFields)
        if (xe(e, s.field.name) === !0) {
  ...
  async _stat(e, t, s, i, r, a, n) {
    let l = !1,
      u = this._reformulateWithoutAdaptions(t);
    if (
      ((l = u.cannot),
      (t = u.where),
      (u = this._reformulateWithoutAdaptions(r)),
      (l = l || u.cannot),
      (r = u.where) !== null
        ? this._extraFilter !== null && (r = j(this._extraFilter, r))
        : (r = this._extraFilter),
      l === !0)
    )
      return r === null && s === '' && i === null
  ...
  async _canDoAggregates(e, t, s, i, r) {
    ...
    for (let l = 0; l < e.length; l++)
      for (const u of this.adaptedFields)
        if (e[l].toLowerCase() === u.field.name.toLowerCase() && !(u instanceof se)) return !1;
    const a = [];
    for (let l = 0; l < t.length; l++) {
      ...
      if (u.workingexpr !== null) {
        ...
        const d = u.clone();
        (d.workingexpr = o.where), a.push(d);
      } else a.push(u);
    }
    ...
    return (
      ...
      ((r = n.where) !== null
        ? this._extraFilter !== null && (r = j(this._extraFilter, r))
        : (r = this._extraFilter),
      this._parent._canDoAggregates(e, a, s, i, r))
    );
  }
  async _getAggregatePagesDataSourceDefinition(e, t, s, i, r, a, n) {
    if (this._parent === null) throw new C(T.NeverReach);
    ...
    for (let o = 0; o < t.length; o++) {
      ...
      if (d.workingexpr !== null) {
        ...
        if (c.cannot) throw new C(T.NeverReach);
        const f = d.clone();
        (f.workingexpr = c.where), l.push(f);
      } else l.push(d);
    }
    const u = this._reformulateWithoutAdaptions(r);
    if (u.cannot) throw new C(T.NeverReach);
    return (
      (r = u.where) !== null
        ? this._extraFilter !== null && (r = j(this._extraFilter, r))
        : (r = this._extraFilter),
      this._parent._getAggregatePagesDataSourceDefinition(e, l, s, i, r, a, n)
    );
  }
};
function Te(h, e) {
...
class W {
  ...
  order(e) {
    e.sort((t, s) => {
      for (let i = 0; i < this._fields.length; i++) {
        const r = this.featureValue(t.feature, this._fields[i], i),
          a = this.featureValue(s.feature, this._fields[i], i);
        let n = 0;
        if (((n = this._directions[i] === 1 ? Te(r, a) : -1 * Te(r, a)), n !== 0)) return n;
      }
      return 0;
    });
  }
  scanForField(e) {
    for (let t = 0; t < this._fields.length; t++)
      if (this._fields[t].toLowerCase().trim() === e.toLowerCase().trim()) return !0;
    return !1;
  }
  replaceFields(e) {
    let t = '';
    for (let s = 0; s < this._fields.length; s++) {
      ...
      for (const r of e)
        if (i.toLowerCase() === r.field.toLowerCase()) {
          i = r.newfield;
          break;
        }
  ...
  featureValue(e, t, s) {
    ...
    for (const r in e.attributes)
      if (t.toLowerCase() === r.toLowerCase()) return (this._fields[s] = r), e.attributes[r];
    return null;
  }
}
let ie = class Le extends L {
  constructor(e) {
    super(e),
  ...
  async manualOrderSet(e, t) {
    ...
    (r = this._orderbyclause) == null || r.order(s);
    const i = new p([], [], !0, null);
    for (let a = 0; a < s.length; a++) i._known.push(s[a].id);
    return i;
  }
  async getIdColumnDictionary(e, t, s, i) {
    if (s < e._known.length - 1) {
      const r = this._maxQueryRate();
      if (e._known[s + 1] === 'GETPAGES')
        return (
          await Q(this._parent._expandPagedSet(e, r, 0, 0, i)),
          this.getIdColumnDictionary(e, t, s, i)
        );
      let a = s + 1;
      const n = [];
      for (; a < e._known.length && e._known[a] !== 'GETPAGES'; ) n.push(e._known[a]), a++;
      s += n.length;
      const l = await Q(this._parent._getFeatureBatch(n, i));
      this._checkCancelled(i);
    ...
    return e._candidates.length > 0
      ? (await Q(this._refineSetBlock(e, this._maxProcessingRate(), i)),
        this._checkCancelled(i),
        this.getIdColumnDictionary(e, t, s, i))
      : t;
  }
  ...
  static registerAction() {
    L._featuresetFunctions.orderBy = function (e) {
      return e === '' ? this : new Le({ parentfeatureset: this, orderbyclause: new W(e) });
    };
  }
  getFieldsIndex() {
    return this._parent.getFieldsIndex();
...
function yt(h) {
  if (h.parseTree.type === 'function') {
    if (h.parseTree.args.value.length === 0) return { name: h.parseTree.name, expr: null };
    if (h.parseTree.args.value.length > 1) throw new x(N.MissingStatisticParameters);
    const e = D.create(nt(h.parseTree.args.value[0], k.Standardised, h.parameters), {
      fieldsIndex: h.fieldsIndex,
      timeZone: h.timeZone,
...
let De = class re {
  ...
  static parseStatField(e, t, s, i) {
    const r = new re();
    r.field = e;
    const a = D.create(t, { fieldsIndex: s, timeZone: i }),
      n = yt(a);
    if (n === null) throw new x(N.UnsupportedSqlFunction, { function: '' });
    const l = n.name.toUpperCase().trim();
    if (l === 'MIN') {
      if (((r.typeofstat = 'MIN'), (r.workingexpr = n.expr), a === null))
        throw new x(N.InvalidFunctionParameters, { function: 'min' });
    } else if (l === 'MAX') {
      if (((r.typeofstat = 'MAX'), (r.workingexpr = n.expr), a === null))
        throw new x(N.InvalidFunctionParameters, { function: 'max' });
    } else if (l === 'COUNT') (r.typeofstat = 'COUNT'), (r.workingexpr = n.expr);
    else if (l === 'STDEV') {
      if (((r.typeofstat = 'STDDEV'), (r.workingexpr = n.expr), a === null))
        throw new x(N.InvalidFunctionParameters, { function: 'stdev' });
    } else if (l === 'SUM') {
      if (((r.typeofstat = 'SUM'), (r.workingexpr = n.expr), a === null))
        throw new x(N.InvalidFunctionParameters, { function: 'sum' });
    } else if (l === 'MEAN') {
      if (((r.typeofstat = 'AVG'), (r.workingexpr = n.expr), a === null))
        throw new x(N.InvalidFunctionParameters, { function: l });
    } else if (l === 'AVG') {
      if (((r.typeofstat = 'AVG'), (r.workingexpr = n.expr), a === null))
        throw new x(N.InvalidFunctionParameters, { function: 'avg' });
    } else {
      if (l !== 'VAR') throw new x(N.UnsupportedSqlFunction, { function: l });
      if (((r.typeofstat = 'VAR'), (r.workingexpr = n.expr), a === null))
        throw new x(N.InvalidFunctionParameters, { function: 'var' });
    }
    return r;
  }
  toStatisticsName() {
...
let gt = class Ae extends L {
  ...
  _initialiseFeatureSet() {
    ...
    let t = !1,
      s = 1;
    const i = this._parent ? this._parent.getFieldsIndex() : new Ue([]);
    ...
    for (const a of this._config.statsfields) {
      ...
      (n.field = a.name),
        ...
        (n.workingexpr =
          a.expression instanceof D
            ? a.expression
            : D.create(a.expression, {
                fieldsIndex: i,
                timeZone: this.dateFieldsTimeZoneDefaultUTC,
              })),
        (n.typeofstat = _t(a.statistic)),
    ...
    (r.field = this._nextUniqueName(e)),
      ...
      (r.workingexpr = D.create(this._parent.objectIdField, {
    ...
    for (const a of this._decodedGroupbyfield) {
      ...
      if (
        ...
        if (!l) throw new C(T.AggregationFieldNotFound);
        (a.name = l.name),
        ...
        n.type = this._convertToEsriFieldType(lt(a.expression, this._parent.fields));
        const l = new M();
        (l.name = a.name),
          (l.alias = l.name),
          this.phsyicalgroupbyfields.push(a.name),
          this._adaptedFields.push(new ft(l, a.expression)),
    ...
    if (this._adaptedFields.length > 0)
      for (const a of this._parent.fields) this._adaptedFields.push(new se(a));
    for (let a = 0; a < this._decodedStatsfield.length; a++) {
      const n = new M();
      let l = null;
      const u = this._decodedStatsfield[a];
      (u.field = this._nextUniqueName(e)),
        u.tofieldname === this.objectIdField && (this._internalObjectIdField = u.field),
        (n.name = u.tofieldname),
        (n.alias = n.name);
      const o = u.workingexpr !== null && Z(u.workingexpr) ? R(u.workingexpr, k.Standardised) : '';
      switch (this._decodedStatsfield[a].typeofstat) {
        case 'SUM':
          if (o !== '') {
            if (((l = this._parent.getField(o)), !l)) throw new C(T.AggregationFieldNotFound);
            n.type = l.type;
          } else n.type = 'double';
          break;
        case 'MIN':
        ...
        case 'AVG':
          if (o !== '' && ((l = this._parent.getField(o)), !l))
            throw new C(T.AggregationFieldNotFound);
          n.type = 'double';
      }
      this.fields.push(n);
    }
  ...
  async _getFeatures(e, t, s, i) {
    ...
    return this._checkIfNeedToExpandKnownPage(e, r) === !0
      ? (await this._expandPagedSet(e, r, 0, 0, i), this._getFeatures(e, t, s, i))
      : 'success';
  }
  async _getFilteredSet(e, t, s, i, r) {
    if (e !== '') return new p([], [], !0, null);
    let a = null;
    const n = { ordered: !1, nowhereclause: !1 };
    if ((await this._ensureLoaded(), s !== null)) {
      for (let u = 0; u < this._decodedStatsfield.length; u++)
        if (xe(s, this._decodedStatsfield[u].tofieldname) === !0) {
    ...
    if (i !== null) {
      n.ordered = !0;
      for (let u = 0; u < this._decodedStatsfield.length; u++)
        if (i.scanForField(this._decodedStatsfield[u].tofieldname) === !0) {
          (i = null), (n.ordered = !1);
          break;
        }
      if (i !== null) {
        for (const u of this._decodedGroupbyfield)
          if (u.singlefield === null && i.scanForField(u.tofieldname) === !0) {
            (i = null), (n.ordered = !1);
            break;
          }
      }
    ...
    if (
      ...
      return (
        ...
        (a =
          n.nowhereclause === !0
            ? new p(
                d._candidates.slice().concat(d._known.slice()),
                [],
    ...
    if (
      ...
      a = new p(['GETPAGES'], [], !1, {
        ...
        internal: {
          ...
          iterator: null,
          set: [],
          subfeatureset: new ie({
            parentfeatureset: l,
            orderbyclause: new W(
              this.phsyicalgroupbyfields.join(',') + ',' + this._parent.objectIdField + ' ASC',
            ),
          }),
        },
      });
      ...
      if (s !== null) {
        let o = null;
        s && (o = this._reformulateWhereClauseWithoutGroupByFields(s)),
          (u = new B({ parentfeatureset: u, whereclause: o }));
      }
      a = new p(['GETPAGES'], [], !1, {
        aggregatefeaturesetpagedefinition: !0,
        resultOffset: 0,
  ...
  _reformulateWhereClauseWithoutStatsFields(e) {
    for (const t of this._decodedStatsfield)
      e = V(e, t.tofieldname, R(t.workingexpr, k.Standardised), this._parent.getFieldsIndex());
    return e;
  }
  _reformulateWhereClauseWithoutGroupByFields(e) {
    for (const t of this._decodedGroupbyfield)
      t.tofieldname !== t.name &&
        (e = V(e, t.tofieldname, R(t.expression, k.Standardised), this._parent.getFieldsIndex()));
    return e;
  }
  _reformulateOrderClauseWithoutGroupByFields(e) {
    const t = [];
    for (const s of this._decodedGroupbyfield)
      s.tofieldname !== s.name && t.push({ field: s.tofieldname, newfield: s.name });
  ...
  async _refineSetBlock(e, t, s) {
    return this._checkIfNeedToExpandCandidatePage(e, this._maxQuery) === !0
      ? (await this._expandPagedSet(e, this._maxQuery, 0, 0, s), this._refineSetBlock(e, t, s))
      : (this._checkCancelled(s),
  ...
  async _getPhysicalPage(e, t, s) {
    if (e.pagesDefinition.aggregatefeaturesetpagedefinition === !0)
      return this._sequentialGetPhysicalItem(e, e.pagesDefinition.resultRecordCount, s, []);
    ...
    for (const r of i) {
      ...
      for (const l of this._decodedGroupbyfield)
        a.attributes[l.tofieldname] = n[l.name.toLowerCase()];
      for (const l of this._decodedStatsfield)
        a.attributes[l.tofieldname] = n[l.field.toLowerCase()];
      this._featureCache[a.attributes[this.objectIdField]] = new q(a);
    }
    return i.length;
  }
    ...
    return new Promise((r, a) => {
      e.pagesDefinition.internal.iterator === null &&
        (e.pagesDefinition.internal.iterator =
          e.pagesDefinition.internal.subfeatureset.iterator(s)),
        e.pagesDefinition.internal.fullyResolved === !0 || t === 0
          ...
          : this._nextAggregateItem(
              ...
              (n) => {
                n === null
                  ? r(i.length)
                  : ((t -= 1), r(this._sequentialGetPhysicalItem(e, t, s, i)));
              },
              a,
            );
    });
  ...
  _nextAggregateItem(e, t, s, i, r, a) {
    try {
      Q(e.pagesDefinition.internal.iterator.next()).then((n) => {
        if (n === null)
          if (e.pagesDefinition.internal.workingItem !== null) {
            ...
            i.push(l),
              ...
              e.pagesDefinition.internal.set.push(l.attributes[this.objectIdField]),
          ...
          if (e.pagesDefinition.internal.workingItem === null)
            ...
            if (l !== e.pagesDefinition.internal.workingItem.id) {
              const u = this._calculateAndAppendAggregateItem(
                e.pagesDefinition.internal.workingItem,
              );
              return (
                i.push(u),
                (e.pagesDefinition.internal.workingItem = null),
                e.pagesDefinition.internal.set.push(u.attributes[this.objectIdField]),
                (t -= 1),
                (e.pagesDefinition.internal.workingItem = { features: [n], id: l }),
                void r(u)
              );
  ...
  _calculateFieldStat(e, t, s) {
    ...
    for (const r of e.features)
      if (t.workingexpr !== null) {
        ...
        a !== null &&
          (a instanceof Me || a instanceof Be
            ? i.push(a.toNumber())
            : a instanceof dt
              ? i.push(a.toMilliseconds())
              : i.push(a));
      } else i.push(null);
    s.attributes[t.tofieldname] = ht(t.typeofstat, [i]);
  }
  _calculateAndAppendAggregateItem(e) {
    const t = { attributes: {}, geometry: null };
    for (const i of this._decodedGroupbyfield) {
      const r = i.singlefield
        ? e.features[0].attributes[i.singlefield]
        : D.convertValueToStorageFormat(i.expression.calculateValue(e.features[0]), i.sqlType);
    ...
    for (let i = 0; i < this._decodedStatsfield.length; i++)
      s.push(this._calculateFieldStat(e, this._decodedStatsfield[i], t));
    return (
      (this._featureCache[t.attributes[this.objectIdField]] = new q({
  ...
  static registerAction() {
    L._featuresetFunctions.groupby = function (e, t) {
      return new Ae({ parentfeatureset: this, groupbyfields: e, statsfields: t });
    };
  }
};
class oe extends L {
  ...
  async _getSet(e) {
    if (this._wset === null) {
      ...
      return (
        ...
        this._setKnownLength(this._wset) >= this._topnum && (this._wset._candidates = []),
        this._wset
      );
    }
    return this._wset;
  ...
  _isInFeatureSet(e) {
    ...
    return s === S.InFeatureSet || s === S.NotInFeatureSet
      ? s
      : t === S.InFeatureSet && s === void 0
        ? this._countedin < this._topnum
          ? ((this._idstates[e] = S.InFeatureSet), this._countedin++, S.InFeatureSet)
          : ((this._idstates[e] = S.NotInFeatureSet), S.NotInFeatureSet)
        : S.Unknown;
  }
  async _expandPagedSet(e, t, s, i, r) {
    if (this._parent === null) throw new C(T.NotImplemented);
    if (
      ...
      return (
        ...
        n > 0 && e._candidates[n - 1] === 'GETPAGES' && (e._candidates.length = n - 1),
        'success'
      );
    }
    const a = await this._parent._expandPagedSet(e, t, s, i, r);
    return (
      this._setKnownLength(e) > this._topnum && (e._known.length = this._topnum),
  ...
  async _getFeatures(e, t, s, i) {
    ...
    let n = 0;
    for (
      let o = e._lastFetchedIndex;
      o < e._known.length &&
      (n++,
      n <= s && (e._lastFetchedIndex += 1),
      !(
        this._featureCache[e._known[o]] === void 0 &&
        (e._known[o] !== t && r.push(e._known[o]), r.length > a)
      ));
      o++
    );
    if (r.length === 0) return 'success';
    const l = new p([], r, !1, null),
      u = Math.min(r.length, s);
    await this._parent._getFeatures(l, -1, u, i);
    for (let o = 0; o < u; o++) {
      const d = this._parent._featureFromCache(r[o]);
      d !== void 0 && (this._featureCache[r[o]] = d);
    }
    return 'success';
  }
  async _getFilteredSet(e, t, s, i, r) {
  ...
  _refineKnowns(e, t) {
    ...
    this._setKnownLength(e) > this._topnum && (e._known = e._known.slice(0, this._topnum)),
  ...
  static registerAction() {
    L._featuresetFunctions.top = function (e) {
      return new oe({ parentfeatureset: this, topnum: e });
    };
  }
  getFieldsIndex() {
    return this._parent.getFieldsIndex();
...
class J extends L {
  constructor(e) {
    super(e),
      (this.declaredClass = 'esri.arcade.featureset.sources.FeatureLayerDynamic'),
      ...
      e.includeGeometry !== void 0 && (this._removeGeometry = e.includeGeometry === !1);
  }
  _maxQueryRate() {
    return le;
  }
  ...
  convertQueryToLruCacheKey(e) {
    const t = this.urlQueryPath + ',' + We(e.toJSON());
    return ve(t, Re.String);
  }
  async loadImpl() {
  ...
  _initialiseFeatureSet() {
    var e, t, s, i, r, a;
    if (
      (this.spatialReference == null && (this.spatialReference = this._layer.spatialReference),
      (this.geometryType = this._layer.geometryType ?? ''),
      (this.fields = this._layer.fields.slice()),
      (this.hasZ =
        ((s =
          (t = (e = this._layer) == null ? void 0 : e.capabilities) == null ? void 0 : t.data) ==
        null
          ? void 0
          : s.supportsZ) === !0),
      (this.hasM =
        ((a =
          (r = (i = this._layer) == null ? void 0 : i.capabilities) == null ? void 0 : r.data) ==
        null
          ? void 0
          : a.supportsM) === !0),
      this._overrideFields !== null)
      ...
      if (this._overrideFields.length === 1 && this._overrideFields[0] === '*')
        ...
        for (const u of this.fields)
          if (u.type === 'oid') n.push(u), l.push(u.name);
          else
    ...
    if (this._layer.source && this._layer.source.sourceJSON) {
      ...
      this._layer.source.sourceJSON.useStandardizedQueries === !0
        ? ((this._databaseType = k.StandardisedNoInterval),
          n != null && n >= 10.61 && (this._databaseType = k.Standardised))
        : n != null &&
          (n >= 10.5 &&
            ((this._databaseType = k.StandardisedNoInterval), (this._requestStandardised = !0)),
          n >= 10.61 && (this._databaseType = k.Standardised));
    }
    this.objectIdField = this._layer.objectIdField;
    for (const n of this.fields) n.type === 'global-id' && (this.globalIdField = n.name);
    (this.subtypeField = this._layer.subtypeField ?? ''),
      (this.subtypes = this._layer.subtypes),
      (this.typeIdField = ('typeIdField' in this._layer ? this._layer.typeIdField : null) ?? ''),
      (this.types = 'types' in this._layer ? this._layer.types : null);
  }
  _isInFeatureSet() {
    return S.InFeatureSet;
  }
  ...
  get gdbVersion() {
    return this._layer &&
      ...
      ? this._layer.gdbVersion || 'SDE.DEFAULT'
      : '';
  }
  nativeCapabilities() {
    return {
  ...
  _createQuery() {
    ...
    return (
      (e.returnZ = this.hasZ),
      (e.returnM = this.hasM),
      this.datesInUnknownTimezone && (e.timeReferenceUnknownClient = !0),
      this._requestStandardised && (e.sqlFormat = 'standard'),
      this._useDefinitionExpression
        ? this._layer.type === 'subtype-group' && (e.where = this._layer.definitionExpression)
        : (e.where = null),
      e
    );
  }
  executeQuery(e, t) {
    const s =
      t === 'execute'
        ? this._layer.queryFeatures.bind(this._layer)
        : t === 'executeForCount'
          ? this._layer.queryFeatureCount.bind(this._layer)
          : this._layer.queryObjectIds.bind(this._layer);
    let i = null;
    if (this.recentlyUsedQueries) {
      const r = this.convertQueryToLruCacheKey(e);
      (i = this.recentlyUsedQueries.getFromCache(r)),
        i === null &&
          ((i = s(e)),
          this.recentlyUsedQueries.addToCache(r, i),
          (i = i.catch((a) => {
            var n;
            throw ((n = this.recentlyUsedQueries) == null || n.removeFromCache(r), a);
          })));
    }
    return (
  ...
  async _getFilteredSet(e, t, s, i, r) {
    ...
    if (this.isTable() && t && e !== null && e !== '') return new p([], [], !0, null);
    ...
    i !== null &&
      ...
      ((n = i.constructClause()), (l = !0));
    const u = this._createQuery();
    (u.where = E(u.where, s === null ? (t === null ? '1=1' : '') : R(s, a))),
      (u.spatialRelationship = this._makeRelationshipEnum(e)),
      (u.outSpatialReference = this.spatialReference),
      (u.orderByFields = n !== '' ? n.split(',') : null),
      (u.geometry = t === null ? null : t),
      (u.relationParameter = this._makeRelationshipParam(e));
    let o = await this.executeQuery(u, 'executeForIds');
    return o === null && (o = []), this._checkCancelled(r), new p([], o, l, null);
  }
  _expandPagedSet(e, t, s, i, r) {
    return this._expandPagedSetFeatureSet(e, t, s, i, r);
  }
  async _getFilteredSetUsingPaging(e, t, s, i, r) {
    ...
    i !== null &&
      ...
      this._layer.capabilities.query &&
      this._layer.capabilities.query.supportsOrderBy === !0 &&
      ((a = i.constructClause()), (n = !0));
    const l = await this.databaseType(),
      u = s === null ? (t === null ? '1=1' : '') : R(s, l);
  ...
  async _getPhysicalPage(e, t, s) {
    ...
    (n.spatialRelationship = e.pagesDefinition.spatialRel),
      ...
      (n.where = E(n.where, e.pagesDefinition.where)),
      (n.orderByFields =
        e.pagesDefinition.orderByFields !== '' ? e.pagesDefinition.orderByFields.split(',') : null),
    ...
    for (let o = 0; o < l.features.length; o++)
      e.pagesDefinition.internal.set[r + o] = l.features[o].attributes[u];
    ...
    return (
      ((l.exceededTransferLimit === void 0 &&
        ...
        (e.pagesDefinition.internal.fullyResolved = !0),
      (e.pagesDefinition.internal.lastRetrieved = i + l.features.length),
  ...
  async _getFeatures(e, t, s, i) {
    ...
    if (
      ...
      return (
        await this._expandPagedSet(e, this._maxProcessingRate(), 0, 0, i),
        this._getFeatures(e, t, s, i)
      );
    let a = 0;
    for (let o = e._lastFetchedIndex; o < e._known.length; o++) {
      if (((e._lastFetchedIndex += 1), a++, this._featureCache[e._known[o]] === void 0)) {
        ...
        if (
          d === !1 &&
          (e._known[o] !== t && r.push(e._known[o]), r.length >= this._maxProcessingRate() - 1)
        )
          break;
      }
    ...
    (n.objectIds = r),
      (n.outFields = this._overrideFields ?? this._fieldsIncludingObjectId(['*'])),
      (n.returnGeometry = !0),
      this._removeGeometry === !0 && (n.returnGeometry = !1),
      (n.outSpatialReference = this.spatialReference);
    const l = await this.executeQuery(n, 'execute');
    if ((this._checkCancelled(i), l.error !== void 0))
      throw new C(T.RequestFailed, { reason: l.error });
    const u = this._layer.objectIdField;
    for (let o = 0; o < l.features.length; o++)
      this._featureCache[l.features[o].attributes[u]] = l.features[o];
    return 'success';
  ...
  async _getDistinctPages(e, t, s, i, r, a, n, l, u) {
    ...
    let d = s.parseTree.column;
    const c = this._layer.fields ?? [];
    for (let _ = 0; _ < c.length; _++)
      if (c[_].name.toLowerCase() === d.toLowerCase()) {
        d = c[_].name;
        break;
      }
    const f = this._createQuery();
    (f.where = E(f.where, a === null ? (r === null ? '1=1' : '') : R(a, o))),
      (f.spatialRelationship = this._makeRelationshipEnum(i)),
      (f.relationParameter = this._makeRelationshipParam(i)),
      (f.geometry = r === null ? null : r),
      (f.returnDistinctValues = !0),
      (f.returnGeometry = !1),
      (f.outFields = [d]);
    const y = await this.executeQuery(f, 'execute');
    if ((this._checkCancelled(u), !y.hasOwnProperty('features')))
      throw new C(T.InvalidStatResponse);
    let I = !1;
    for (let _ = 0; _ < c.length; _++)
      if (c[_].name === d) {
        c[_].type === 'date' && (I = !0);
    ...
    for (let _ = 0; _ < y.features.length; _++) {
      if (I) {
        const F = y.features[_].attributes[d];
        F !== null ? l.push(new Date(F)) : l.push(F);
      } else l.push(y.features[_].attributes[d]);
      if (l.length >= n) break;
    }
    return y.features.length === 0
      ? l
      : y.features.length ===
            ((b = this._layer.capabilities) == null ? void 0 : b.query.maxRecordCount) &&
          l.length < n
        ? {
            calculated: !0,
            result: await this._getDistinctPages(e + y.features.length, t, s, i, r, a, n, l, u),
          }
        : l;
  }
  ...
  async _stats(e, t, s, i, r, a, n) {
    var v;
    await this._ensureLoaded();
    const l = (v = this._layer.capabilities) == null ? void 0 : v.query,
      u = !!(l != null && l.supportsSqlExpression),
      o = !!(l != null && l.supportsStatistics),
      d = !!(l != null && l.supportsDistinct);
    if (e === 'count') return d ? this._countstat(e, s, i, r) : { calculated: !1 };
    if (o === !1 || (Z(t) === !1 && u === !1) || t.isStandardized === !1)
      return s !== '' || r !== null ? { calculated: !1 } : this._manualStat(e, t, a, n);
    if (e === 'distinct')
      return d === !1
        ? s !== '' || r !== null
          ? { calculated: !1 }
          : this._manualStat(e, t, a, n)
        : this._distinctStat(e, t, s, i, r, a, n);
    const c = await this.databaseType();
    if (this.isTable() && i && s !== null && s !== '') return { calculated: !0, result: null };
    const f = this._createQuery();
    (f.where = E(f.where, r === null ? (i === null ? '1=1' : '') : R(r, c))),
      (f.spatialRelationship = this._makeRelationshipEnum(s)),
      (f.relationParameter = this._makeRelationshipParam(s)),
      (f.geometry = i === null ? null : i);
    const y = new me();
    (y.statisticType = ot(e)),
      (y.onStatisticField = R(t, c)),
      (y.outStatisticFieldName = 'ARCADE_STAT_RESULT'),
      (f.returnGeometry = !1);
    let I = 'ARCADE_STAT_RESULT';
    f.outStatistics = [y];
    const b = await this.executeQuery(f, 'execute');
    if (!b.hasOwnProperty('features') || b.features.length === 0)
      throw new C(T.InvalidStatResponse);
    let _ = !1;
    const F = b.fields ?? [];
    for (let m = 0; m < F.length; m++)
      if (F[m].name.toUpperCase() === 'ARCADE_STAT_RESULT') {
        (I = F[m].name), F[m].type === 'date' && (_ = !0);
        break;
      }
    if (_) {
      let m = b.features[0].attributes[I];
      return (
        m !== null && (m = new Date(b.features[0].attributes[I])), { calculated: !0, result: m }
      );
    }
    return { calculated: !0, result: b.features[0].attributes[I] };
  }
  _stat(e, t, s, i, r, a, n) {
  ...
  async _canDoAggregates(e, t) {
    ...
    await this._ensureLoaded();
    let s = !1;
    const i = (a = this._layer.capabilities) == null ? void 0 : a.query,
      r = (i == null ? void 0 : i.supportsSqlExpression) === !0;
    if ((i != null && i.supportsStatistics === !0 && i.supportsOrderBy === !0 && (s = !0), s))
      for (let l = 0; l < t.length - 1; l++)
        (((n = t[l].workingexpr) == null ? void 0 : n.isStandardized) === !1 ||
          (Z(t[l].workingexpr) === !1 && r === !1)) &&
          (s = !1);
    return s !== !1;
  }
  ...
  async _getAggregatePagesDataSourceDefinition(e, t, s, i, r, a, n) {
    ...
    a !== null &&
      ...
      this._layer.capabilities.query &&
      this._layer.capabilities.query.supportsOrderBy === !0 &&
      ((u = a.constructClause()), (d = !0)),
      this._layer.capabilities &&
        this._layer.capabilities.query &&
        this._layer.capabilities.query.supportsPagination === !1 &&
        ((d = !1), (o = !0), (u = this._layer.objectIdField));
    const c = [];
    for (let _ = 0; _ < t.length; _++) {
      const F = new me();
      (F.onStatisticField = t[_].workingexpr !== null ? R(t[_].workingexpr, l) : ''),
        (F.outStatisticFieldName = t[_].field),
        (F.statisticType = t[_].toStatisticsName()),
        c.push(F);
    }
    u === '' && (u = e.join(','));
    ...
    y != null && y < f && (f = y);
    const I = r === null ? (i === null ? '1=1' : '') : R(r, l);
    return new p([], ['GETPAGES'], d, {
      groupbypage: !0,
      spatialRel: this._makeRelationshipEnum(s),
      relationParam: this._makeRelationshipParam(s),
      outFields: ['*'],
      useOIDpagination: o,
      generatedOid: n,
      resultRecordCount: f,
      ...
      outStatistics: c,
      geometry: i === null ? null : i,
      where: I,
      orderByFields: u,
      returnGeometry: !1,
      returnIdsOnly: !1,
      internal: { lastMaxId: -1, set: [], lastRetrieved: 0, lastPage: 0, fullyResolved: !1 },
    });
  }
  async _getAgregagtePhysicalPage(e, t, s) {
    ...
    e.pagesDefinition.useOIDpagination === !0 &&
      (i = E(
        i,
        e.pagesDefinition.generatedOid + '>' + e.pagesDefinition.internal.lastMaxId.toString(),
      ));
    ...
    if (
      ...
      (l.groupByFieldsForStatistics = e.pagesDefinition.groupByFieldsForStatistics),
    ...
    if ((this._checkCancelled(s), !u.hasOwnProperty('features')))
      throw new C(T.InvalidStatResponse);
    const o = [];
    if (e.pagesDefinition.internal.lastPage !== n) return [];
    u.features.length > 0 &&
      u.features[0].attributes[e.pagesDefinition.generatedOid] === void 0 &&
      (e.pagesDefinition.generatedOid = e.pagesDefinition.generatedOid.toLowerCase());
    for (let d = 0; d < u.features.length; d++)
      e.pagesDefinition.internal.set[a + d] =
        u.features[d].attributes[e.pagesDefinition.generatedOid];
    for (let d = 0; d < u.features.length; d++)
      o.push(new q({ attributes: u.features[d].attributes, geometry: null }));
    return (
      e.pagesDefinition.useOIDpagination === !0
        ? u.features.length === 0
          ? (e.pagesDefinition.internal.fullyResolved = !0)
          : (e.pagesDefinition.internal.lastMaxId =
              u.features[u.features.length - 1].attributes[e.pagesDefinition.generatedOid])
        : ((u.exceededTransferLimit === void 0 &&
            u.features.length !== e.pagesDefinition.resultRecordCount) ||
            u.exceededTransferLimit === !1) &&
          (e.pagesDefinition.internal.fullyResolved = !0),
      (e.pagesDefinition.internal.lastRetrieved = r + u.features.length),
      (e.pagesDefinition.internal.lastPage += e.pagesDefinition.resultRecordCount),
      o
    );
  }
  static create(e, t, s, i, r) {
    const a = new G({ url: e, outFields: t === null ? ['*'] : t });
    return new J({ layer: a, spatialReference: s, lrucache: i, interceptor: r });
  }
  relationshipMetaData() {
    var e;
    return this._layer &&
      this._layer.source &&
      (e = this._layer.source.sourceJSON) != null &&
      e.relationships
      ? this._layer.source.sourceJSON.relationships
      : [];
  }
  serviceUrl() {
    return ke(this._layer.parsedUrl.path);
  }
  async queryAttachments(e, t, s, i, r) {
    function a(l) {
      const u = l.capabilities;
      return (
        (u == null ? void 0 : u.data.supportsAttachment) &&
        (u == null ? void 0 : u.operations.supportsQueryAttachments)
      );
    }
    const n = this._layer;
    if (a(n)) {
      const l = { objectIds: [e], returnMetadata: r };
      ((t && t > 0) || (s && s > 0)) && (l.size = [t && t > 0 ? t : 0, s && s > 0 ? s : t + 1]),
        i && i.length > 0 && (l.attachmentTypes = i),
        this.featureSetQueryInterceptor &&
          this.featureSetQueryInterceptor.preLayerQueryCallback({
            layer: n,
            query: l,
      ...
      const u = await n.queryAttachments(new Qe(l)),
        o = [];
      return (
        u &&
          u[e] &&
          u[e].forEach((d) => {
            const c =
              this._layer.parsedUrl.path + '/' + e.toString() + '/attachments/' + d.id.toString();
            let f = null;
            r && d.exifInfo && (f = He.convertJsonToArcade(d.exifInfo, 'system', !0)),
              o.push(new Ye(d.id, d.name, d.contentType, d.size, c, f, d.keywords ?? null));
          }),
        o
      );
    }
  ...
  async queryRelatedFeatures(e) {
    ...
    const t = {
      ...
      outFields: (i = e.outFields) == null ? void 0 : i.join(','),
    ...
    e.resultOffset !== void 0 &&
      ...
      e.objectIds && e.objectIds.length > 0 && (t.objectIds = e.objectIds.join(',')),
      e.outSpatialReference && (t.outSR = Ve(e.outSpatialReference)),
      this.featureSetQueryInterceptor &&
    ...
    const s = await U(this._layer.parsedUrl.path + '/queryRelatedRecords', {
      responseType: 'json',
      query: t,
    });
    if (s.data) {
      const r = {},
        a = s.data;
      if (a != null && a.relatedRecordGroups) {
        ...
        for (const l of a.relatedRecordGroups) {
          ...
          for (const d of l.relatedRecords) {
            ...
            const c = new q({
              geometry: d.geometry ? Ze(d.geometry) : null,
              attributes: d.attributes,
            });
            o.push(c);
          }
          r[u] = { features: o, exceededTransferLimit: a.exceededTransferLimit === !0 };
        }
      }
      return r;
    }
    throw new C(T.InvalidRequest);
  }
  async getFeatureByObjectId(e, t) {
    ...
    (s.outFields = t),
      ...
      (s.where = E(s.where, this.objectIdField + '=' + e.toString())),
      this.featureSetQueryInterceptor &&
        this.featureSetQueryInterceptor.preLayerQueryCallback({
          layer: this._layer,
          query: s,
    ...
    const i = await this._layer.queryFeatures(s);
    return i.features.length === 1 ? i.features[0] : null;
  }
  async getIdentityUser() {
    var t;
    await this.load();
    const e = (t = we) == null ? void 0 : t.findCredential(this._layer.url);
    return e ? e.userId : null;
  }
  async getOwningSystemUrl() {
    var i, r;
    await this.load();
    const e = (i = we) == null ? void 0 : i.findServerInfo(this._layer.url);
    if (e) return e.owningSystemUrl;
    let t = this._layer.url;
    const s = t.toLowerCase().indexOf('/rest/services');
    if (((t = s > -1 ? t.slice(0, s) : t), t)) {
      t += '/rest/info';
      try {
        const a = await U(t, { query: { f: 'json' } });
        let n = '';
        return (r = a.data) != null && r.owningSystemUrl && (n = a.data.owningSystemUrl), n;
      } catch {
        return '';
      }
    }
  ...
  async getFeatureSetInfo() {
    ...
    if (s) {
      const n = await U(s, { responseType: 'json', query: { f: 'json' } });
      (e = ((r = n == null ? void 0 : n.data) == null ? void 0 : r.name) ?? null),
        (t = ((a = n == null ? void 0 : n.data) == null ? void 0 : a.serviceItemId) ?? null);
    }
    ...
    return (
      (this.featureSetInfo = {
        ...
        layerName: e === '' ? null : e,
        itemId: t === '' ? null : t,
        serviceLayerUrl: s === '' ? null : s,
        webMapLayerId: i ? (this._layer.id ?? null) : null,
        webMapLayerTitle: i ? (this._layer.title ?? null) : null,
        className: null,
        objectClassId: null,
      }),
      this.fsetInfo
...
class K extends L {
  constructor(e) {
    super(e),
      (this.declaredClass = 'esri.arcade.featureset.sources.FeatureLayerMemory'),
      (this._removeGeometry = !1),
      (this._overrideFields = null),
    ...
    if (
      ...
      if (this._overrideFields.length === 1 && this._overrideFields[0] === '*')
        ...
        const n = [],
          l = [];
        for (const u of this.fields)
          if (u.type === 'oid' || this._layer.objectIdField === u.name) n.push(u), l.push(u.name);
          else
            for (const o of this._overrideFields)
              if (o.toLowerCase() === u.name.toLowerCase()) {
                n.push(u), l.push(u.name);
    ...
    (this._databaseType = k.Standardised),
      ...
      (this.hasM =
        ...
        null
          ? void 0
          : a.supportsM) === !0),
      (this.subtypeField = ('subtypeField' in this._layer ? this._layer.subtypeField : null) ?? ''),
      (this.subtypes = 'subtypes' in this._layer ? this._layer.subtypes : null),
      (this.typeIdField = ('typeIdField' in this._layer ? this._layer.typeIdField : null) ?? ''),
      (this.types = 'types' in this._layer ? this._layer.types : null);
  }
  isTable() {
    return (
      this._forceIsTable ||
      ('isTable' in this._layer && this._layer.isTable) ||
      this._layer.type === 'table' ||
      !this._layer.geometryType
    );
  }
  ...
  _createQuery() {
    ...
    return (
      (e.returnZ = this.hasZ),
      (e.returnM = this.hasM),
      (e.outFields = this._overrideFields ?? ['*']),
      (e.returnGeometry = !this._removeGeometry),
      e
    );
  }
  _changeFeature(e) {
    ...
    return new q({ geometry: this._removeGeometry === !0 ? null : e.geometry, attributes: t });
  }
  async _getFilteredSet(e, t, s, i, r) {
    let a = '',
      n = !1;
    if (
      (i !== null && ((a = i.constructClause()), (n = !0)),
      this.isTable() && t && e !== null && e !== '')
    )
      return new p([], [], !0, null);
    const l = this._createQuery();
    (l.where = E(l.where, s === null ? (t === null ? '1=1' : '') : R(s, k.Standardised))),
  ...
  async _queryAllFeatures() {
    ...
    if ((await this._ensureLoaded(), this._layer.source && this._layer.source.items)) {
      ...
      return (
        ...
        (this._wset = new p([], i, !1, null)),
        this._wset
      );
    }
    const e = this._createQuery();
    e.where = '1=1';
  ...
  async _getFeatures(e, t, s) {
    ...
    for (
      ...
      !(
        ...
        (e._known[r] !== t && i.push(e._known[r]), i.length > s)
      ));
      r++
    );
    if (i.length === 0) return 'success';
    throw new C(T.MissingFeatures);
  }
  async _refineSetBlock(e) {
    return e;
  }
  ...
  static create(e, t) {
    ...
    const i = e.layerDefinition.typeIdField ?? '',
      r = [];
    if (e.layerDefinition.types) for (const m of e.layerDefinition.types) r.push(Je.fromJSON(m));
    ...
    if (!s) {
      let m = !1;
      for (const O of e.layerDefinition.fields)
        if (O.type === 'oid' || O.type === 'esriFieldTypeOID') {
          (s = O.name), (m = !0);
          break;
        }
      if (m === !1) {
        ...
        for (; he; ) {
          let P = !0;
          for (const Pe of e.layerDefinition.fields)
            if (Pe.name === O) {
              P = !1;
              break;
            }
          P === !0 ? (he = !1) : (ce++, (O = 'FID' + ce.toString()));
        }
        e.layerDefinition.fields.push({ type: 'esriFieldTypeOID', name: O, alias: O });
        const H = [];
    ...
    const u = [];
    for (const m of e.layerDefinition.fields) m instanceof M ? u.push(m) : u.push(M.fromJSON(m));
    let o = a;
    switch ((o || (o = ''), o)) {
    ...
    if (o !== 'esriGeometryNull')
      for (const m of n)
        m.geometry &&
          !(m.geometry instanceof Ke) &&
          ((m.geometry.type = o),
          m.geometry.spatialReference === void 0 && (m.geometry.spatialReference = l));
    else for (const m of n) m.geometry && (m.geometry = null);
    const d = {
        outFields: ['*'],
        source: n,
        fields: u,
        hasZ:
          ((y = e == null ? void 0 : e.layerDefinition) == null ? void 0 : y.hasZ) === !0 ||
          ((I = e == null ? void 0 : e.featureSet) == null ? void 0 : I.hasZ) === !0,
        hasM:
          ((b = e == null ? void 0 : e.layerDefinition) == null ? void 0 : b.hasM) === !0 ||
          ((_ = e == null ? void 0 : e.featureSet) == null ? void 0 : _.hasM) === !0,
        types: r,
        typeIdField: i,
        objectIdField: s,
        spatialReference: t,
      },
      c = o === 'esriGeometryNull' || o === null;
    c || (d.geometryType = o);
    const f = new G(d);
    return (
      (F = e == null ? void 0 : e.layerDefinition) != null &&
        F.subtypeField &&
        (v = e == null ? void 0 : e.layerDefinition) != null &&
        v.subtypes &&
        f.read({
          subtypes: e.layerDefinition.subtypes,
          subtypeField: e.layerDefinition.subtypeField,
        }),
      new K({ layer: f, spatialReference: t, isTable: c })
    );
  }
...
class mt extends L {
  constructor(e) {
    super(e),
      (this.declaredClass = 'esri.arcade.featureset.sources.FeatureLayerRelated'),
  ...
  async loadImpl() {
    var e;
    return (
      await Promise.all([this._layer.load(), (e = this._relatedLayer) == null ? void 0 : e.load()]),
  ...
  _initialiseFeatureSet() {
    if (
      ...
      (this.hasZ = this._relatedLayer.hasZ),
      (this.hasM = this._relatedLayer.hasM),
      this._overrideFields !== null)
    )
      if (this._overrideFields.length === 1 && this._overrideFields[0] === '*')
        this._overrideFields = null;
        ...
        for (const i of this.fields)
          if (i.type === 'oid') t.push(i), s.push(i.name);
          else
    ...
    e &&
      ((this._databaseType = e.databaseType), (this._requestStandardised = e.requestStandardised)),
      ...
      (this.subtypeField = this._relatedLayer.subtypeField),
  ...
  async _getFilteredSet(e, t, s, i, r) {
    ...
    if (a.canQueryRelated === !1) return new p([], [], !0, null);
    if ((_ = a.capabilities) != null && _.queryRelated.supportsPagination)
      return this._getFilteredSetUsingPaging(e, t, s, i, r);
    let n = '',
      l = !1;
    i !== null &&
      ((F = a.capabilities) == null ? void 0 : F.queryRelated.supportsOrderBy) === !0 &&
      ((n = i.constructClause()), (l = !0));
    const u = new Fe();
    ...
    const o =
      this._overrideFields !== null
        ...
        : this._fieldsIncludingObjectId(
            this._relatedLayer.fields ? this._relatedLayer.fields.map((v) => v.name) : ['*'],
          );
    (u.outFields = o), (u.relationshipId = this.relationship.id), (u.where = '1=1');
    let d = !0;
    this._removeGeometry === !0 && (d = !1),
      (u.returnGeometry = d),
      this._requestStandardised && (u.sqlFormat = 'standard'),
      (u.outSpatialReference = this.spatialReference),
      (u.orderByFields = n !== '' ? n.split(',') : null);
    const c = await a.source.queryRelatedFeatures(u);
    this._checkCancelled(r);
    const f = c[this._findObjectId] ? c[this._findObjectId].features : [],
      y = [];
    for (let v = 0; v < f.length; v++)
      (this._featureCache[f[v].attributes[this._relatedLayer.objectIdField]] = f[v]),
        y.push(f[v].attributes[this._relatedLayer.objectIdField]);
    const I = t && e !== null && e !== '',
      b = s != null;
    return new p(I || b ? y : [], I || b ? [] : y, l, null);
  }
  _fieldsIncludingObjectId(e) {
    if (e === null) return [this.objectIdField];
    const t = e.slice();
  ...
  async _getFilteredSetUsingPaging(e, t, s, i, r) {
    ...
    i !== null &&
      ((_ = l.capabilities) == null ? void 0 : _.queryRelated.supportsOrderBy) === !0 &&
      ((a = i.constructClause()), (n = !0)),
      await this.databaseType();
    const u = '1=1';
    let o = this._maxQueryRate();
    const d = (F = l.capabilities) == null ? void 0 : F.query.maxRecordCount;
    d != null && d < o && (o = d);
    const c = t && e !== null && e !== '',
      f = s != null;
    let y = null,
    ...
    return (
      (y = new p(c || f ? ['GETPAGES'] : [], c || f ? [] : ['GETPAGES'], n, {
        outFields: b.join(','),
        resultRecordCount: o,
        resultOffset: 0,
      ...
      await this._expandPagedSet(y, o, 0, 0, r),
      y
    );
  }
  _expandPagedSet(e, t, s, i, r) {
  ...
  async _getPhysicalPage(e, t, s) {
    ...
    for (let c = 0; c < o.length; c++)
      e.pagesDefinition.internal.set[r + c] = o[c].attributes[this._relatedLayer.objectIdField];
    for (let c = 0; c < o.length; c++)
    ...
    return (
      o.length !== e.pagesDefinition.resultRecordCount &&
        d &&
        (e.pagesDefinition.internal.fullyResolved = !0),
      (e.pagesDefinition.internal.lastRetrieved = i + o.length),
      (e.pagesDefinition.internal.lastPage += e.pagesDefinition.resultRecordCount),
      o.length
    );
  }
  async _getFeatures(e, t, s, i) {
    ...
    for (
      ...
      !(
        ...
        (e._known[l] !== t && r.push(e._known[l]), r.length > s)
      )) &&
      !(n >= s && r.length === 0);
      l++
    );
...
async function z(h, e, t) {
  if (g.applicationCache) {
    const s = g.applicationCache.getLayerInfo(h);
    if (s) {
      const a = await s;
      return new G({ url: h, outFields: e, sourceJSON: a });
    }
    const i = new G({ url: h, outFields: e }),
      r = (async () => (await i.load(), i.sourceJSON))();
    if (g.applicationCache) {
      g.applicationCache.setLayerInfo(h, r);
      try {
        return await r, i;
      } catch (a) {
        throw (g.applicationCache.clearLayerInfo(h), a);
      }
    }
    return await r, i;
  }
  if (t != null) {
    const s = t.getCachedLayerMetadata(h);
    if (s) {
      const a = await s;
      return new G({ url: h, outFields: e, sourceJSON: a });
    }
    const i = new G({ url: h, outFields: e }),
      r = (async () => (await i.load(), i.sourceJSON))();
    t.setCachedLayerMetadata(h, r);
    try {
      return await r, i;
    } catch (a) {
      throw (t.removeCachedLayerMetadata(h, r), a);
    }
  }
  return new G({ url: h, outFields: e });
}
async function ue(h, e, t, s, i, r = null) {
  return A(await z(h, ['*'], i), e, t, s, i, r);
}
function A(h, e = null, t = null, s = !0, i = null, r = null) {
  switch (h.type) {
    case 'catalog-footprint':
      return A(h.parent, e, t, s, i, r);
    case 'subtype-sublayer': {
      const a = A(h.parent, e, t, s, i, r);
      return a.filter(
        D.create(h.parent.subtypeField + '=' + h.subtypeCode.toString(), {
          fieldsIndex: h.parent.fieldsIndex,
    ...
    case 'subtype-group': {
      ...
      return !h.url && h.source ? new K(a) : new J(a);
    }
    default:
      throw new Error(`Unsupported layer type: ${h.type}`);
  }
}
async function Ft(h) {
  if (g.applicationCache !== null) {
    const t = g.applicationCache.getLayerInfo(h);
    if (t !== null) return t;
  }
  const e = (async () => {
    const t = await U(h, { responseType: 'json', query: { f: 'json' } });
    return t.data ? t.data : null;
  })();
  if (g.applicationCache !== null) {
    g.applicationCache.setLayerInfo(h, e);
    try {
      return await e;
    } catch (t) {
      throw (g.applicationCache.clearLayerInfo(h), t);
    }
  }
  return e;
}
async function St(h, e) {
  const t = 'QUERYDATAELEMTS:' + e.toString() + ':' + h;
  if (g.applicationCache !== null) {
    const i = g.applicationCache.getLayerInfo(t);
    if (i !== null) return i;
  }
  const s = (async () => {
    var r;
    const i = await U(h + '/queryDataElements', {
      method: 'post',
      responseType: 'json',
      query: { layers: JSON.stringify([e.toString()]), f: 'json' },
    });
    if (i.data) {
      const a = i.data;
      if ((r = a.layerDataElements) != null && r[0]) return a.layerDataElements[0];
    }
    throw new C(T.DataElementsNotFound);
  })();
  if (g.applicationCache !== null) {
    g.applicationCache.setLayerInfo(t, s);
    try {
      return await s;
    } catch (i) {
      throw (g.applicationCache.clearLayerInfo(t), i);
    }
  }
  return s;
}
async function Oe(h, e) {
  if (g.applicationCache !== null) {
    const s = g.applicationCache.getLayerInfo(h);
    if (s !== null) return s;
  }
  if (e != null) {
    const s = e.getCachedServiceMetadata(h);
    if (s != null) return s;
  }
  const t = (async () => {
    const s = await U(h, { responseType: 'json', query: { f: 'json' } });
    if (s.data) {
      const i = s.data;
  ...
  if (g.applicationCache !== null) {
    g.applicationCache.setLayerInfo(h, t);
    try {
      return await t;
    } catch (s) {
      throw (g.applicationCache.clearLayerInfo(h), s);
    }
  }
  if (e != null) {
    e.setCachedServiceMetadata(h, t);
    try {
      return await t;
    } catch (s) {
      throw (e.removeCachedServiceMetadata(h, t), s);
    }
  }
  return t;
}
async function It(h, e, t) {
  var r, a, n;
  const s = {
      metadata: null,
      networkId: -1,
  ...
  if (
    ((s.metadata = i),
    ((r = i.controllerDatasetLayers) == null ? void 0 : r.utilityNetworkLayerId) !== void 0 &&
      i.controllerDatasetLayers.utilityNetworkLayerId !== null)
  ) {
    if (i.layers) for (const o of i.layers) s.layerNameLkp[o.id] = o.name;
    if (i.tables) for (const o of i.tables) s.layerNameLkp[o.id] = o.name;
    const l = i.controllerDatasetLayers.utilityNetworkLayerId;
    s.networkId = l;
    const u = await St(h, l);
    if (u) {
      (s.queryelem = u),
        (a = s.queryelem) != null &&
          a.dataElement &&
          s.queryelem.dataElement.schemaGeneration !== void 0 &&
          (s.unVersion = s.queryelem.dataElement.schemaGeneration),
        (s.lkp = {}),
        s.queryelem.dataElement.domainNetworks || (s.queryelem.dataElement.domainNetworks = []);
      for (const d of s.queryelem.dataElement.domainNetworks) {
        for (const c of d.edgeSources ?? []) {
          const f = {
            layerId: c.layerId,
            sourceId: c.sourceId,
            className: s.layerNameLkp[c.layerId] ?? null,
          };
          s.layerIdLookup.set(f.layerId, f),
            s.sourceIdLookup.set(f.sourceId, f),
            f.className && (s.lkp[f.className] = f);
        }
        for (const c of d.junctionSources ?? []) {
          const f = {
            layerId: c.layerId,
            sourceId: c.sourceId,
            className: s.layerNameLkp[c.layerId] ?? null,
      ...
      if (s.queryelem.dataElement.terminalConfigurations)
        for (const d of s.queryelem.dataElement.terminalConfigurations)
          for (const c of d.terminals)
            s.terminals.push({ terminalId: c.terminalId, terminalName: c.terminalName });
      const o = await Ft(h + '/' + l);
      if (
        ((n = o.systemLayers) == null ? void 0 : n.associationsTableId) !== void 0 &&
        o.systemLayers.associationsTableId !== null
      ) {
        let d = null;
        if (t && s.unVersion < 8) {
          const c = [];
          s.unVersion >= 4 && (c.push('STATUS'), c.push('PERCENTALONG')),
            (d = await ue(
              h + '/' + o.systemLayers.associationsTableId,
              e,
              [
                'OBJECTID',
                'FROMNETWORKSOURCEID',
            ...
            s.unVersion >= 4 &&
              ((d = d.filter(
                D.create(
                  ...
                  { fieldsIndex: d.getFieldsIndex(), timeZone: d.dateFieldsTimeZoneDefaultUTC },
                ),
              )),
              await d.load());
        }
        return {
          lkp: s.lkp,
          associations: d,
          unVersion: s.unVersion,
          terminals: s.terminals,
          layerIdLookup: s.layerIdLookup,
          sourceIdLookup: s.sourceIdLookup,
        };
      }
      return {
        associations: null,
...
async function bt(h, e, t, s = null, i = null, r = !0, a = null, n = null) {
  let l = h.serviceUrl();
  if (!l) return null;
  l =
    l.charAt(l.length - 1) === '/'
...
class Ct extends Ne {
  constructor(e, t = null, s = null, i = null) {
    super(),
      (this._map = e),
      (this._overrideSpatialReference = t),
      (this._lrucache = s),
      (this._interceptor = i),
      (this._instantLayers = []);
  }
  _makeAndAddFeatureSet(e, t = !0, s = null) {
    ...
    return (
      this._instantLayers.push({
        ...
        opitem: e,
        includeGeometry: t,
        outFields: JSON.stringify(s),
      }),
      i
    );
  }
  async featureSetByName(e, t = !0, s = null) {
    if (Se.isLoadable(this._map) && !this._map.loaded)
      return await this._map.load(), this.featureSetByName(e, t, s);
    s === null && (s = ['*']), (s = (s = s.slice()).sort());
    const i = JSON.stringify(s);
    for (let a = 0; a < this._instantLayers.length; a++) {
      const n = this._instantLayers[a];
      if (n.opitem.title === e && n.includeGeometry === t && n.outFields === i)
        return this._instantLayers[a].featureset;
    }
    const r = this._map.allLayers.find((a) => $(a) && a.title === e);
    if (r != null) return this._makeAndAddFeatureSet(r, t, s);
    if (this._map.tables) {
      const a = this._map.tables.find((n) => $(n) && n.title === e);
      if (a != null) return this._makeAndAddFeatureSet(a, t, s);
    }
    return null;
  }
  async featureSetById(e, t = !0, s = ['*']) {
    if (Se.isLoadable(this._map) && !this._map.loaded)
      return await this._map.load(), this.featureSetById(e, t, s);
    s === null && (s = ['*']), (s = (s = s.slice()).sort());
    const i = JSON.stringify(s);
    for (let a = 0; a < this._instantLayers.length; a++) {
      const n = this._instantLayers[a];
      if (n.opitem.id === e && n.includeGeometry === t && n.outFields === i)
        return this._instantLayers[a].featureset;
    }
    const r = this._map.allLayers.find((a) => $(a) && a.id === e);
    if (r) return this._makeAndAddFeatureSet(r, t, s);
    if (this._map.tables) {
      const a = this._map.tables.find((n) => $(n) && n.id === e);
      if (a != null) return this._makeAndAddFeatureSet(a, t, s);
    }
    return null;
  }
...
class de extends Ne {
  ...
  _makeAndAddFeatureSet(e, t = !0, s = null) {
    const i = A(e, this._overrideSpatialReference, s === null ? ['*'] : s, t, this._lrucache);
    return (
      this._instantLayers.push({
        featureset: i,
        opitem: e,
  ...
  async _loadMetaData() {
    const e = await Oe(this._url, this._lrucache);
  ...
  clone() {
    return new de(this._url, this._overrideSpatialReference, this._lrucache, this._interceptor);
  }
  async featureSetByName(e, t = !0, s = null) {
    s === null && (s = ['*']), (s = (s = s.slice()).sort());
    const i = JSON.stringify(s);
    for (let n = 0; n < this._instantLayers.length; n++) {
      ...
      if (l.opitem.title === e && l.includeGeometry === t && l.outFields === i)
  ...
  async featureSetById(e, t = !0, s = ['*']) {
    ...
    for (let n = 0; n < this._instantLayers.length; n++) {
      const l = this._instantLayers[n];
      if (l.opitem.id === e && l.includeGeometry === t && l.outFields === i)
        return this._instantLayers[n].featureset;
    }
    const r = await this._loadMetaData();
    let a = null;
...
function Tt(h, e, t = null, s = null) {
  return new Ct(h, e, t, s);
}
function Dt(h, e, t = null, s = null) {
  return new de(h, e, t, s);
}
function vt(h, e, t, s, i) {
  ...
  if (h instanceof te && Xe(h)) {
    const r = h;
    switch (e) {
      case 'datasource':
        return A(r, i, 'outFields' in r ? r.outFields : null, !0, t, s).getDataSourceFeatureSet();
      case 'parent':
      case 'root':
        return A(r, i, 'outFields' in r ? r.outFields : null, !0, t, s);
    }
    return null;
  }
  if (tt(h)) {
    switch (e) {
      case 'datasource':
        return A(h.parent, i, h.parent.outFields, !0, t, s).getDataSourceFeatureSet();
      case 'parent':
      case 'root':
        return A(h, i, h.parent.outFields, !0, t, s);
    }
    return null;
  }
  return null;
}
async function Rt(h, e, t, s, i, r, a, n = null) {
  if (g.applicationCache) {
    const u = g.applicationCache.getLayerInfo(h + ':' + r.url);
    if (u) return ee(await u, e, t, s, i, a, n);
  }
  if (a != null) {
    const u = a.getCachedPortalItem(r.url, h);
    if (u != null) return await ee(await u, e, t, s, i, a, n);
  }
  const l = new ze({ id: h, portal: r }).load();
  g.applicationCache
    ? g.applicationCache.setLayerInfo(h + ':' + r.url, l)
    : a != null && a.setCachedPortalItem(r.url, h, l);
  try {
    return await ee(await l, e, t, s, i, a, n);
  } catch (u) {
    throw (
      (g.applicationCache && g.applicationCache.clearLayerInfo(h + ':' + r.url),
      a != null && a.removeCachedPortalItem(r.url, h, l),
      u)
    );
  }
}
async function ee(h, e, t, s, i, r, a) {
  let n;
  if (h.type === 'Feature Service' || h.type === 'Map Service')
    n = await z(ke(h.url ?? '') + '/' + e, ['*'], r);
  else {
    if (e) throw new Error(`layerId=${e} provided for ${h.type} item`);
    if (r != null) {
      const l = r.getCachedPortalItemLayer(h.portal.url, h.id);
      if (l != null) n = await l;
      else {
        const u = te.fromPortalItem(h);
        r.setCachedPortalItemLayer(h.portal.url, h.id, u);
        try {
          n = await u;
        } catch (o) {
          throw (r.removeCachedPortalItemLayer(h.portal.url, h.id, u), o);
        }
      }
    } else n = await te.fromPortalItem(h);
  }
  return A(n, t, s, i, r, a);
}
const jt = Object.freeze(
  Object.defineProperty(
    {
      __proto__: null,
      constructAssociationMetaDataFeatureSetFromUrl: It,
      constructFeatureSet: A,
      constructFeatureSetFromPortalItem: Rt,
      constructFeatureSetFromRelationship: bt,
      constructFeatureSetFromUrl: ue,
      convertToFeatureSet: vt,
      createFeatureSetCollectionFromMap: Tt,
      createFeatureSetCollectionFromService: Dt,
      initialiseMetaDataCache: wt,
    },
    Symbol.toStringTag,
    { value: 'Module' },
  ),
